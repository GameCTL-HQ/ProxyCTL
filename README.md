...
🌐 **[proxyctl.cc](https://proxyctl.cc)** &nbsp;·&nbsp; 🚀 **`v0.6.17`** &nbsp;·&nbsp; 🔗 **Sibling: [GameCTL](https://gamectl.cc)** — deploys & manages the game servers ProxyCTL publishes
...
Neither plane needs a credential you hand over. ProxyCTL generates **its own**
ed25519 SSH key for the droplet (the private half never leaves the app, and no
API returns it), and reaches your cluster through its own in-cluster
ServiceAccount — read-only for discovery, namespaced for the gateways it
manages. Nothing runs in the background: SSH and `kubectl` fire only when you
click Apply. Full model: [`SECURITY.md`](SECURITY.md).
...
| Env | Default | Purpose |
|---|---|---|
| `PROXYCTL_HOST` | — | UI hostname for Ingress mode (must resolve to your ingress controller). |
| `PROXYCTL_EXPOSE` | `auto` | How to expose the UI: `auto` detects what your cluster has; override to force `ingress` or a LoadBalancer. |
| `PROXYCTL_INGRESS_CLASS` | auto-detected | `ingressClassName` to use when exposing via Ingress. |
| `PROXYCTL_IMAGE` | `ghcr.io/gamectl-hq/proxyctl:latest` | Override the image — e.g. a private/homelab registry. |
| `PROXYCTL_NAMESPACE` | `proxyctl` | Namespace to install into. |
| `PROXYCTL_STORAGE_CLASS` | `nfs-ssd` | StorageClass for ProxyCTL's own data PVC. |
| `PROXYCTL_DATA_NFS` | — | `server:/path` — headless escape hatch that deploys straight onto an NFS share, skipping the wizard's Storage step. |
| `PROXYCTL_MANIFEST` | fetched from the public repo | Path or URL to an alternate `k8s/proxyctl.yaml`. |
| `PROXYCTL_ASSUME_YES` | `0` | Set to `1` to skip interactive prompts (CI/automation). |
...
1. **☁️ A DigitalOcean droplet** — the cheapest **$4/mo Basic** tier is plenty for most home game servers; size up only if you actually saturate it. Sign up: **[digitalocean.com](https://www.digitalocean.com/)**.
   - The exact plan to pick when creating the droplet:
     ...
     | Auth | **Add your own SSH key** at creation (recommended — it's how *you* get a root shell). ProxyCTL doesn't use it: the Setup wizard generates ProxyCTL's own keypair and gives you a one-line command to install its public half |
...
That's it. **You do not configure anything on the droplet by hand.** The only
manual step is pasting one command that installs ProxyCTL's generated public
key — from there its in-app Setup wizard SSHes in and installs `wireguard` /
`iptables` / `conntrack`, persists kernel sysctls, generates the WireGuard
keypair, and brings up `wg-quick@wg0`. Idempotent — safe to re-run any time.
...
- **No credentials handed over** — ProxyCTL uses its own generated SSH key (private half never leaves the app, never returned by any API) and its own namespaced ServiceAccount. It never holds your `ssh-agent`, your kubeconfig, or cluster-admin.
- **Hardened SSH, guided** — one-click control tunnel, tunnel-only SSH lockdown, fail2ban with a "let me back in" button, and detection of leftover manual UFW rules that would strand you on an IP change.
- **Your own WireGuard access** — register personal devices as tunnel peers (QR code for phones, one-paste block for headless Linux) so lockdown never locks *you* out.
- **Monitoring built in** — per-entry heartbeat bars and latency trends, history that survives restarts, and Discord alerts when something goes unreachable.
...
- **No credential of yours is stored.** ProxyCTL generates its own ed25519
  droplet key at setup; the private half lives 0600 on its PVC and is never
  returned by an API, logged, or sent to the browser. It never holds your
  `ssh-agent`, your kubeconfig, or cluster-admin.
- **Never sees a WireGuard private key.** Each gateway pod self-generates its
  own keypair on a small PVC — ProxyCTL reads back only the public half. The
  droplet's own private key is re-spliced *on the droplet* and never crosses
  the wire.
- **The Cloudflare token is memory-only** unless you set `CF_API_TOKEN`. Never
  written to disk by default, never logged, never sent to the browser.
- The auth Secret (`proxyctl-auth`) is written by ProxyCTL itself at first-run
  setup via its own ServiceAccount — same bootstrap-token flow as GameCTL.
- Cluster picker is **read-only** (`kubectl get` verbs only), only fires on
  click, never in the background.
- **Nothing runs unattended.** There is no reconcile loop — SSH and `kubectl`
  execute only when you click Apply, and every command's exit code and
  verbatim stdout/stderr is shown back to you.

Full model — RBAC footprint, the two SSH-lockdown options, fail2ban, and the
leftover-UFW-rule recovery: [`SECURITY.md`](SECURITY.md).
...
- This repo is a clean evolution of the internal `gameproxy` app — the in-process userspace forwarder is gone, replaced by the tunnel. ProxyCTL doesn't manage UFW day-to-day, but the Setup wizard can detect and safely remove a leftover pre-ProxyCTL UFW rule that restricts SSH to a specific IP (see [`SECURITY.md`](SECURITY.md)).
- The UI is React 19 + Vite, built to `server/web/dist` and compiled into the binary with `//go:embed` — so the deploy shape stays a single artifact, exactly like GameCTL. Styling is hand-written CSS rather than Tailwind, tracking GameCTL's look so a future merge stays natural.
- **Apply is scoped and incremental.** Proxy Entries and Web Apps apply independently, and an apply reconciles only the gateways that are new, edited, or missing a key — the ones it skipped are named. **Full reconcile** forces everything through, for state that drifted outside ProxyCTL.
- Entries and web routes are **editable in place**; changing a label no longer tears down a tunnel.
