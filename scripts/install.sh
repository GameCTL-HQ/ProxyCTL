...
# --- Storage: decided in the GUI, not here ----------------------------------
# A fresh install boots on an emptyDir ("setup mode") and the FIRST step of
# the in-app wizard asks where everything lives: the operator names an NFS
# share, TESTS it from a probe pod, and adopts it — ProxyCTL then patches its
# own Deployment onto the share (app/ beside the gateway Keys/) and restarts.
# A cluster-storage fallback (StorageClass PVC) lives in the same screen.
#
# Headless escape hatch: PROXYCTL_DATA_NFS="server:/path" skips setup mode
# and deploys straight onto the share (same layout the GUI adopt produces).
DATA_NFS="${PROXYCTL_DATA_NFS:-}"
DATA_VOLUME_SPEC="emptyDir: {}"
DATA_SUBPATH=""; DATA_NFS_SERVER=""; DATA_NFS_EXPORT=""
if [ -n "$DATA_NFS" ]; then
  case "$DATA_NFS" in
    *:/*) DATA_NFS_SERVER="${DATA_NFS%%:*}"; DATA_NFS_EXPORT="${DATA_NFS#*:}" ;;
    *) die "PROXYCTL_DATA_NFS must look like server:/path (got: $DATA_NFS)" ;;
  esac
  DATA_NFS_EXPORT="${DATA_NFS_EXPORT%/}"
  DATA_VOLUME_SPEC="nfs: { server: ${DATA_NFS_SERVER}, path: ${DATA_NFS_EXPORT} }"
  DATA_SUBPATH="app"
  say "Install storage: NFS ${DATA_NFS_SERVER}:${DATA_NFS_EXPORT} (headless — skipping the GUI storage step)"
else
  say "Install storage: chosen in the app — the wizard's Storage step runs first"
fi
...
# Only offered for NodePort. Not for LoadBalancer: picking MetalLB is an
# explicit "no DNS needed" choice, so re-asking about a hostname there just
# contradicts what they picked. Not offered for port-forward either, where
# an Ingress would contradict the choice.
ALSO_INGRESS=0
if [ "$EXPOSE" = "nodeport" ] && [ "$HAS_INGRESS" -eq 1 ]; then
...
sed -e "s/__IMAGE__/$(esc "$IMAGE")/g" \
    ...
    -e "s/__DATA_VOLUME_SPEC__/$(esc "$DATA_VOLUME_SPEC")/g" \
