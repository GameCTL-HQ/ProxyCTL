...
const (
	// trafficSampleEvery is the SSH cadence for the byte-counter read
	// (WireGuard peer counters + droplet WAN), which drives the persistent
	// totals, the quota bar and the quota alerts. 10s keeps the UI numbers
	// moving without real extra droplet load — the 5s /api/stats poll already
	// SSHes the droplet whenever a page is open.
	trafficSampleEvery = 10 * time.Second
	// healthEveryTicks: the heavier work (kube endpoint checks, TCP latency
	// dials, web-route HTTPS probes, uptime-history appends) runs once every
	// N traffic ticks — 60s — so reachability resolution, history ring size
	// and probe load keep their original cadence.
	healthEveryTicks = 6
	// Ring length comes from the configured retention (default 30d, see
	// AlertConfig.RetentionDays) at the 60s health cadence. Held in memory and
	// snapshotted to /data (next to entries.json) every 5 min + on
	// shutdown, so history survives a pod restart.
	uptimeSnapshotEveryTicks = 30
)
...
// self-corrected on the next health tick a minute later).
func (a *API) StartUptimeSampler(ctx context.Context) {
	...
	go func() {
		...
		t := time.NewTicker(trafficSampleEvery)
		defer t.Stop()
		ticks := 0
		for {
			select {
			case <-ctx.Done():
				...
				return
			case <-t.C:
				a.sampleUptime(ctx, ticks%healthEveryTicks == 0)
...
// wgPeer is one gateway peer's live counters from `wg show wg0 dump`. The
// peer lines are tab-separated: pubkey, preshared-key, endpoint, allowed-ips,
...
// sampleUptime runs one tick. The SSH byte-counter read + traffic record +
// quota evaluation happen every tick (10s); `full` (every 6th tick, 60s)
// additionally runs the reachability/latency checks, web-route probes and
// appends the uptime-history samples.
func (a *API) sampleUptime(ctx context.Context, full bool) {
	now := time.Now().Unix()

	entrySamples := map[string]UptimeSample{}
	if sa, ok := a.applier.(SSHApplier); ok && sa.dropletReady() {
		// One ssh call carries BOTH the WireGuard peer counters and the
		// droplet's WAN byte counters — the WAN read is appended after a
		// __WAN__ marker line so the two blocks parse apart. The WAN
		// interface comes from the wizard config, falling back to the
		// default-route interface (covers DO's eth0 out of the box).
		iface := "eth0"
		if dc := a.droplet.Get(); dc != nil && dc.WANIface != "" {
			iface = dc.WANIface
		}
		remote := "wg show wg0 dump; echo __WAN__; IF='" + iface + "'; " +
		...
		st := sa.sshDroplet("uptime-sample", "", remote)

		peers := map[string]wgPeer{} // gateway pubkey -> handshake + byte counters
		trafficRaw := map[string]rawBytes{}
		var wanRx, wanTx int64
		wanN := -1 // -1 = before the __WAN__ marker; >= 0 = byte lines seen after it
		for _, ln := range strings.Split(st.Stdout, "\n") {
			...
			f := strings.Split(ln, "\t")
			if len(f) < 7 { // interface line / anything non-peer
				continue
			}
			hs, _ := strconv.ParseInt(f[4], 10, 64)
			rx, _ := strconv.ParseInt(f[5], 10, 64)
			tx, _ := strconv.ParseInt(f[6], 10, 64)
			peers[f[0]] = wgPeer{hs: hs, rx: rx, tx: tx}
		}
		haveWAN := wanN >= 2 // both rx and tx lines actually read
		for _, e := range a.store.List() {
			if !e.Enabled || e.GatewayPubKey == "" {
				continue
			}
			p, present := peers[e.GatewayPubKey]
			hs := p.hs
			if present {
				trafficRaw[e.ID] = rawBytes{rx: p.rx, tx: p.tx}
			}
			if !full { // traffic ticks skip the reachability/latency work
				continue
			}
		...
		// Persist byte totals (per-entry + WAN) for this tick, roll the
		// monthly cycle, and fire any quota alert. WAN is only recorded when
		// the read actually produced both counters, so a transient miss
		// leaves the last good values untouched instead of corrupting them.
		if a.traffic != nil {
			live := map[string]bool{}
			for _, e := range a.store.List() {
				live[e.ID] = true
			}
			a.traffic.Record(trafficRaw, rawBytes{rx: wanRx, tx: wanTx}, haveWAN, live, now)

			cfg := a.alerts.Get()
			if cfg.QuotaResetDay > 0 {
				a.traffic.MaybeMonthlyReset(cfg.QuotaResetDay, now)
			}
			if cfg.Enabled && cfg.DiscordWebhookURL != "" && cfg.QuotaBytes > 0 {
				if kinds := a.traffic.EvaluateQuota(cfg.QuotaBytes, cfg.EffectiveQuotaNearPct()); len(kinds) > 0 {
					_, tx := a.traffic.Wantotals()
					// Sequential in one goroutine so a skipped band (e.g. 0→over)
					// lands in Discord as near, then over.
					go func() {
						for _, kind := range kinds {
							fireTrafficAlert(cfg.DiscordWebhookURL, kind, tx, cfg.QuotaBytes)
						}
					}()
				}
			}
