...
func (a *API) generatePersonalAccess(w http.ResponseWriter, r *http.Request) {
	...
	// The name is interpolated into the droplet's wg0.conf as a comment
	// (RenderDropletWG0) and applied by the control-wg sidecar; a stray
	// newline could split that line and inject a config directive. Reject
	// control chars up front, same as Entry.name (store.go).
	if hasControlChars(name) {
		a.writeJSON(w, http.StatusBadRequest, map[string]string{"error": `device name contains control characters (newline, tab, etc.) — use a plain name like "Laptop" or "Phone"`})
		return
	}
