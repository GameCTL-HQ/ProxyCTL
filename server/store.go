...
// L4 protocol (iptables -m multiport needs a single proto per rule).
...
// portConflictLocked returns an error when an ENABLED entry e claims a
// public port/proto that another enabled entry already routes. Every
// enabled entry DNATs on the droplet's single public IP, so one port can
// only ever reach one target — a second claim doesn't error at apply
// time, it just silently loses to iptables first-match (which is how two
// LiveKit entries ended up fighting over the media ports). Sharing a
// SUBDOMAIN across entries is fine and deliberate: DNS just points at
// the droplet, and the ports are what route. Caller holds s.mu.
		...
		for _, p := range e.Ports {
			for _, op := range other.Ports {
				if p.Port == op.Port && protosOverlap(p.Proto, op.Proto) {
					return fmt.Errorf(
						"public port %d/%s is already routed by enabled entry %q — one port can only reach one target (disable that entry or pick a different port)",
						p.Port, p.Proto, other.Name)
				}
			}
		}
	}
...
// own /32 so its tunnel is independent. Caller holds s.mu.
