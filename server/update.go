...
// "update available" to the UI; "apply" rolls ProxyCTL's own Deployment
// (imagePullPolicy: Always + a :latest/moving tag → pulls the newest image).
// The auth Secret, droplet config, and every gateway are untouched, so no
// re-setup is needed.
...
// updateApply rolls ProxyCTL's own Deployment so it re-pulls its image (with
// imagePullPolicy: Always + a moving tag this lands the newest published
// build). Uses the in-cluster kubectl + ProxyCTL's ServiceAccount, which the
// proxyctl-gateway-mgr Role already allows to patch deployments in-namespace.
func (a *API) updateApply(w http.ResponseWriter, r *http.Request) {
	cmd := exec.CommandContext(r.Context(), a.kubectl, "-n", a.selfNS, "rollout", "restart", "deploy/"+a.selfDeploy)
	if out, err := cmd.CombinedOutput(); err != nil {
		writeJSONResp(w, http.StatusInternalServerError, map[string]any{
			"error": "rollout restart failed: " + strings.TrimSpace(string(out)),
		})
		return
	}
	writeJSONResp(w, http.StatusOK, map[string]any{
		...
		"message":    "Update started — ProxyCTL is rolling to the latest image. This page will reconnect in a few seconds.",
	})
}
