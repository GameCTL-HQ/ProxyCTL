...
func (a *API) listWebRoutes(w http.ResponseWriter, r *http.Request) {
	// Attach live reachability from the uptime sampler. The list used to
	// carry config only, so the UI had nothing to colour its status dot
	// with and hard-coded every enabled route to the "down" style —
	// permanently amber, whether the site was up or not. "state" is
	// "up" | "down" | "" (no sample yet: the sampler runs once a minute,
	// and a route added seconds ago has nothing to report).
	routes := a.webroutes.List()
	out := make([]map[string]any, 0, len(routes))
	for _, wr := range routes {
		state := ""
		if h := UptimeHistoryForWebRoute(wr.ID); len(h) > 0 {
			if h[len(h)-1].Reachable {
				state = "up"
			} else {
				state = "down"
			}
		...
		out = append(out, map[string]any{
			"id": wr.ID, "hostname": wr.Hostname, "namespace": wr.Namespace,
			"service": wr.Service, "port": wr.Port, "enabled": wr.Enabled,
			"state": state,
		})
	}
