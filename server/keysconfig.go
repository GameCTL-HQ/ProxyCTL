...
// for why raw): absolute, traversal-free, and every segment restricted to a
// safe charset (letters/digits/. _ -). The value is interpolated into shell
// commands (renderShareCommands → the /etc/exports line applied over SSH on
// the droplet as root) as a single token, so keep shell metacharacters —
// ; $ ` | & ( ) etc. — out entirely. Same allowlist validateKeysBasePath uses.
func validateNFSExportPath(export string) error {
	...
	// Empty segments (leading slash, or "/mnt//ssd") are tolerated here —
	// normalizeNFSExport collapses them — and the bare root "/" is valid.
	for _, seg := range strings.Split(export, "/") {
		...
		for _, r := range seg {
			...
			case r == '.' || r == '_' || r == '-':
			default:
				return fmt.Errorf("NFS export %q has an invalid character %q in segment %q "+
					"(allowed: letters, digits, . _ -)", export, r, seg)
			}
		}
	}
	return nil
