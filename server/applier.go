...
type SSHApplier struct {
	DropletSSH  string // user@host for the droplet, e.g. root@192.0.2.123 (legacy flag fallback)
...
// dropletControlIP is the droplet's fixed WireGuard address (matches
// DefaultInfra's DropletWGIP — a fixed convention of this deployment, not
// operator-configurable). Hardcoded here rather than threaded through
// Infra because several droplet-bound calls (bootstrap, lockdown, test)
// construct SSHApplier without one.
const dropletControlIP = "10.8.0.1"

// sshDropletArgsTo builds the argv every droplet-bound ssh funnels
// through, targeting the given host. With a wizard config it uses -i
// <key>, a proxyCTL-owned known_hosts, and IdentitiesOnly to refuse the
// agent. Without a wizard config, it ignores host and falls back to -o
// BatchMode=yes against the legacy flag target. Privilege is the
// caller's business — see sshDroplet vs sshDropletRaw.
func (s SSHApplier) sshDropletArgsTo(host, remote string) []string {
	var args []string
	if s.Droplet != nil && s.Droplet.Configured() {
...
// sshDropletArgs is sshDropletArgsTo against the normal (public-IP)
// target — the path used by every caller before the control tunnel
// existed, and still the sole path when it isn't configured/ready.
func (s SSHApplier) sshDropletArgs(remote string) []string {
	return s.sshDropletArgsTo(s.dropletTarget(), remote)
}

// controlTarget returns "user@10.8.0.1" — the droplet's address over
// ProxyCTL's own control tunnel — once that tunnel has been set up AND
// verified (Droplet.ControlTunnelReady). Empty otherwise, which is what
// every install that hasn't opted into this feature sees.
func (s SSHApplier) controlTarget() string {
	if s.Droplet != nil && s.Droplet.Configured() && s.Droplet.ControlTunnelReady {
		return s.Droplet.user() + "@" + dropletControlIP
	}
	return ""
}

// verifyControlTunnel proves the control tunnel actually works end to
// end: a direct probe against dropletControlIP, bypassing runDropletSSH's
// normal prefer-then-fallback (which would silently succeed over the
// public IP and give a false positive). Only a Step.OK here should ever
// cause a caller to mark Droplet.ControlTunnelReady.
func (s SSHApplier) verifyControlTunnel() Step {
	probe := s
	probe.Timeout = 8 * time.Second
	target := s.Droplet.user() + "@" + dropletControlIP
	return probe.run("control tunnel: verify", "ssh", "",
		probe.sshDropletArgsTo(target, "echo control-tunnel-ok")...)
}

// runDropletSSH is the chokepoint every droplet-bound ssh path funnels
// through. When the control tunnel is configured and verified, it tries
// that FIRST on a short, hard-bounded timeout — long enough for a real
// handshake's round-trip, short enough that a momentarily-stale tunnel
// (sidecar restarting, handshake not yet refreshed) doesn't stall the
// step. ANY failure there falls straight back to the ssh receiver's own
// Timeout and the public-IP target, i.e. exactly what ran before this
// tunnel existed — the control tunnel can only make an apply step
// resilient to the operator's home IP changing, never the sole way in.
func (s SSHApplier) runDropletSSH(name, stdin, remote string) Step {
	// Once SSH is locked to the control tunnel, the public IP is firewalled
	// off at both sshd and iptables — it's no longer a fallback, it's a
	// dead end. In that state the tunnel is the ONLY way in, so give it the
	// receiver's full timeout (heavy steps like "rebuild NAT chains" on a
	// box with many entries legitimately exceed the 6s race budget) and
	// never fall back to the public IP: doing so would only replace the
	// tunnel's actionable error with a misleading "connect to <public-ip>
	// timed out" and leave the operator staring at the old address they
	// thought they'd moved off of.
	tunnelOnly := s.Droplet != nil && s.Droplet.SSHTunnelOnly
	if ct := s.controlTarget(); ct != "" {
		attempt := s
		if !tunnelOnly {
			// Public IP is still reachable as a fallback, so keep the tunnel
			// probe short — a momentarily-stale tunnel shouldn't stall the
			// step when there's a working alternative.
			attempt.Timeout = 6 * time.Second
		}
		st := attempt.run(name+" [control-tunnel]", "ssh", stdin, s.sshDropletArgsTo(ct, remote)...)
		if st.OK || tunnelOnly {
			return st
		}
	}
	return s.run(name, "ssh", stdin, s.sshDropletArgsTo(s.dropletTarget(), remote)...)
}
...
func (s SSHApplier) sshDroplet(name, stdin, remote string) Step {
	return s.runDropletSSH(name, stdin, s.wrapPrivileged(remote))
}
...
func (s SSHApplier) sshDropletRaw(name, stdin, remote string) Step {
	return s.runDropletSSH(name, stdin, remote)
}
...
func (s SSHApplier) sshDropletLong(name, stdin, remote string, timeout time.Duration) Step {
	...
	return long.runDropletSSH(name, stdin, long.wrapPrivileged(remote))
}
...
// work reports whether this entry's gateway needs reconciling. A nil set
// means "everything" (a full reconcile, or no snapshot to compare against).
func gatewayNeedsWork(work map[string]bool, e *Entry) bool {
	return work == nil || work[e.ID]
}

func (s SSHApplier) ApplyPerGame(in Infra, entries []*Entry, work map[string]bool, persist func(id, pubkey string), onStep func(Step)) ApplyResult {
	...
	// Egress entries have no wg-gw pod (their peer is a sidecar in the game
	// pod, key held there and pubkey supplied on the entry) — they skip the
	// gateway phases entirely and only participate droplet-side.
	var gwEntries []*Entry
	for _, e := range enabled {
		if e.IsEgress() {
			if e.GatewayPubKey == "" {
				add(Step{Name: "game " + e.Name + ": egress peer", OK: false,
					Stderr: "egress entry has no peer public key yet — the orchestrator (GameCTL) supplies it when it injects the game's WireGuard sidecar; re-apply after publishing"})
			}
			continue
		}
		gwEntries = append(gwEntries, e)
	}
	...
	// Phases 1-3 only touch gateways that actually need it (see
	// gatewayWorkSet). Say which ones were skipped rather than silently
	// doing less than the operator expects from a button labelled Apply.
	var skipped []string
	for _, e := range gwEntries {
		if !gatewayNeedsWork(work, e) {
			skipped = append(skipped, e.Name)
		}
	}
	if len(skipped) > 0 {
		add(Step{
			Name:   fmt.Sprintf("skipped %d unchanged gateway(s)", len(skipped)),
			OK:     true,
			Stdout: strings.Join(skipped, ", ") + "\n(use Full reconcile to force these through)",
		})
	}
	...
	// Skipped entries already hold one — gatewayWorkSet forces any entry
	// with an empty GatewayPubKey through, so a skip here can never leave
	// a peer missing from the droplet config below.
	for _, e := range gwEntries {
		if !gatewayNeedsWork(work, e) {
			continue
		}
...
// pushDropletWG0 installs the rendered wg0.conf on the droplet (splicing
// the existing PrivateKey back in — proxyctl never holds the droplet's own
// key) and reconciles the peer set LIVE with `wg syncconf` (no interface
// bounce). Shared by ApplyPerGame's Phase 5 and setupControlTunnel
// (main.go), which needs this exact step without the NAT-chain rebuild
// that follows it in a full per-game apply.
func (s SSHApplier) pushDropletWG0(in Infra, ready []*Entry) Step {
	...
	return s.sshDroplet("droplet: install wg0.conf + wg syncconf (live, no bounce)",
