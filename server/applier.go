...
func (s SSHApplier) ApplyPerGame(in Infra, entries []*Entry, persist func(id, pubkey string), onStep func(Step)) ApplyResult {
	...
	if in.keysShareMode() {
		// Share mode: a static PV + shared RWX PVC on the operator's export.
		// The name is a hash of the share, so a CHANGED share is a different
		// PV rather than a mutation of this one's immutable nfs fields. When
		// the PV already exists (install-time share, or a previous apply)
		// only the namespaced PVC is applied — re-applying a PV needs
		// cluster-scoped RBAC the in-cluster ServiceAccount doesn't have.
		volName := keysShareVolName(in.KeysNFSServer, in.KeysNFSExport)
		if chk := s.run("keys share: check PV "+volName, "kubectl", "",
			kctx("get", "pv", volName)...); chk.OK {
			add(s.run("keys share: ensure PVC "+volName, "kubectl",
				renderKeysSharePVC(in.K8sNamespace, in.KeysNFSServer, in.KeysNFSExport),
				kctx("apply", "-f", "-")...))
		} else {
			add(s.run("keys share: apply PV+PVC "+volName, "kubectl",
				renderKeysSharePV(in.K8sNamespace, in.KeysNFSServer, in.KeysNFSExport),
				kctx("apply", "-f", "-")...))
		}
	} else {
