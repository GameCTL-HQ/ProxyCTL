...
// BatchMode=yes against the legacy flag target.
...
func (s SSHApplier) sshDropletLong(name, stdin, remote string, timeout time.Duration) Step {
	...
	return long.run(name, "ssh", stdin, long.sshDropletArgs(remote)...)
}
...
func (s SSHApplier) ApplyPerGame(in Infra, entries []*Entry, persist func(id, pubkey string), onStep func(Step)) ApplyResult {
	...
	// Phase 0: ensure the keys StorageClass for the configured base folder
	// exists (create-if-missing). Each gateway's key PVC binds via this class,
	// whose pathPattern nests the dir under KeysBasePath on the NFS share. A
	// StorageClass pathPattern is fixed at creation, so a changed base folder
	// is a new class name — created here on the next apply; existing gateways
	// move via the Setup → keys "move" action (delete PVC + re-key).
	...
	for _, e := range enabled {
		add(s.run("game "+e.Name+": rollout",
			"kubectl", "", kctx("-n", ns, "rollout", "status",
				"deploy/wg-gw-"+e.Slug(), "--timeout=120s")...))
	}
