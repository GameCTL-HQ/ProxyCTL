...
func (s SSHApplier) ApplyPerGame(in Infra, entries []*Entry, persist func(id, pubkey string), onStep func(Step)) ApplyResult {
	...
	if in.keysShareMode() {
		...
		// Applied (not create-if-missing) because it's idempotent — the name
		// is a hash of the share, so a CHANGED share is a different PV rather
		// than a mutation of this one's immutable nfs fields.
