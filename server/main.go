...
func main() {
	...
	api := &API{store: store, domains: domains, webroutes: webroutes, cf: cf, kube: kube, infra: DefaultInfra(), applier: applier,
...
func (a *API) testDroplet(w http.ResponseWriter, r *http.Request) {
	...
	sa.Droplet = cfg
	st := sa.sshDroplet("test", "", "echo OK; whoami; uname -n; uptime")
	a.writeJSON(w, http.StatusOK, map[string]any{
		"ok":       st.OK,
...
`
	...
	// Parse the marker line so the wizard can show the WG pubkey AND
