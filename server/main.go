...
func main() {
	...
	// Web tunnels (L7) — hostname → Service routes published as ingress
	// rules on the Cloudflare Tunnel (see cftunnel.go / cloudflared.go).
	// Lives next to entries.json/domains.json.
	webroutes, err := NewWebRouteStore(filepath.Join(filepath.Dir(*dbPath), "webroutes.json"))
	if err != nil {
		log.Fatalf("webroutes store: %v", err)
	}
	// Discord webhook config for reachability alerts. Lives next to
	// entries.json/domains.json like every other small piece of state.
	alertsStore, err := NewAlertConfigStore(filepath.Join(filepath.Dir(*dbPath), "alerts.json"))
	if err != nil {
		log.Fatalf("alerts store: %v", err)
	}
	// Persistent byte-counter totals (per-entry + WAN) and the traffic-quota
	// state. Survives restarts / peer re-keys / entry disable-re-enable.
	trafficStore, err := NewTrafficStore(filepath.Join(filepath.Dir(*dbPath), "traffic-history.json"))
	if err != nil {
		log.Fatalf("traffic store: %v", err)
	}
	...
	api := &API{store: store, domains: domains, keys: keysStore, webroutes: webroutes, alerts: alertsStore, traffic: trafficStore, cf: cf, kube: kube, infra: DefaultInfra(), applier: applier,
		...
		historyPath: filepath.Join(filepath.Dir(*dbPath), "uptime-history.json"),
	...
	protected.HandleFunc("POST /api/droplet/ips", api.setDropletIPs)
	...
	protected.HandleFunc("POST /api/droplet/fail2ban/policy", api.setFail2banPolicy)
	protected.HandleFunc("POST /api/droplet/fail2ban/service", api.setFail2banEnabled)
	...
	protected.HandleFunc("GET /api/entries/{id}/uptime", api.entryUptimeHistory)
	protected.HandleFunc("GET /api/webroutes/{id}/uptime", api.webRouteUptimeHistory)
	protected.HandleFunc("GET /api/monitoring/summary", api.monitoringSummary)
	protected.HandleFunc("GET /api/alerts/config", api.getAlertConfig)
	protected.HandleFunc("PUT /api/alerts/config", api.setAlertConfig)
	protected.HandleFunc("POST /api/alerts/test", api.testAlertConfig)
	protected.HandleFunc("GET /api/traffic/quota", api.trafficQuota)
	protected.HandleFunc("POST /api/traffic/quota/reset", api.resetTrafficQuota)
	...
	// Uptime history: 60s reachability samples + ~1h in-memory history for
	// every enabled Proxy Entry and Web App (Tunnels/Web Apps table
	// sparklines). Its own context so it stops cleanly on shutdown.
	uptimeCtx, uptimeCancel := context.WithCancel(context.Background())
	defer uptimeCancel()
	api.StartUptimeSampler(uptimeCtx)
...
type API struct {
	...
	webroutes   *WebRouteStore    // L7 host routes → Cloudflare Tunnel ingress rules
	alerts      *AlertConfigStore // Discord webhook config for reachability + quota alerts
	traffic     *TrafficStore     // persistent per-entry + WAN byte totals + quota state
	historyPath string            // uptime-history snapshot file (next to entries.json)
	cf          *CF
...
type ApplyJob struct {
	...
	// Scope is which half this apply touched ("all" | "entries" | "webapps"),
	// so a page polling status can tell whether the running job is its own.
...
// Per-half fingerprints. "Pending changes" has to be tracked per half now
// that a page applies only its own: a web-apps apply must not mark edited
// proxy entries as pushed, and vice versa.
func (a *API) entriesHash() string {
	b, _ := json.Marshal(a.store.List())
	s := sha256.Sum256(b)
	return hex.EncodeToString(s[:])
}

func (a *API) webHash() string {
	b, _ := json.Marshal(a.webroutes.List())
	s := sha256.Sum256(b)
	return hex.EncodeToString(s[:])
}

func (a *API) appliedEntriesPath() string {
	return filepath.Join(filepath.Dir(a.appliedPath), "applied-entries.hash")
}

func (a *API) appliedWebPath() string {
	return filepath.Join(filepath.Dir(a.appliedPath), "applied-web.hash")
}

func readHashFile(p string) string {
	b, _ := os.ReadFile(p)
	return strings.TrimSpace(string(b))
}

// markApplied records a successful apply of the given scope. Only the halves
// that actually ran are marked.
func (a *API) markApplied(scope ApplyScope) {
	if scope.doesEntries() {
		_ = os.WriteFile(a.appliedEntriesPath(), []byte(a.entriesHash()), 0o644)
		if b, err := json.Marshal(a.store.List()); err == nil {
			_ = os.WriteFile(a.appliedSnapPath(), b, 0o644)
		}
	}
	if scope.doesWebApps() {
		_ = os.WriteFile(a.appliedWebPath(), []byte(a.webHash()), 0o644)
	}
	// Combined hash stays written for anything still reading the legacy
	// file, but only when BOTH halves are now in sync — otherwise it would
	// claim a clean state a partial apply never reached.
	if readHashFile(a.appliedEntriesPath()) == a.entriesHash() &&
		readHashFile(a.appliedWebPath()) == a.webHash() {
...
func (a *API) pending() bool {
	if len(a.store.List()) == 0 && len(a.webroutes.List()) == 0 && a.appliedHash() == "" {
		return false
	}
	// Upgrade path: installs that applied before per-half tracking have only
	// the combined file. Trust it while the per-half files are absent, so a
	// working install doesn't light up "pending" the moment it updates.
	haveEntries := readHashFile(a.appliedEntriesPath()) != ""
	haveWeb := readHashFile(a.appliedWebPath()) != ""
	if !haveEntries && !haveWeb {
		return a.configHash() != a.appliedHash()
	}
	entriesPending := haveEntries && readHashFile(a.appliedEntriesPath()) != a.entriesHash()
	webPending := haveWeb && readHashFile(a.appliedWebPath()) != a.webHash()
	// A half never applied under the new scheme falls back to the combined
	// comparison rather than silently reporting itself clean.
	if !haveEntries || !haveWeb {
		return entriesPending || webPending || a.configHash() != a.appliedHash()
	}
	return entriesPending || webPending
}
...
// gatewayWorkSet returns the entry IDs whose in-cluster gateway actually
// needs reconciling on this apply — nil meaning "all of them".
//
// Apply is a full declarative reconcile: it walks every entry through
// kubectl apply → rollout status → read-back-public-key, even when only one
// entry changed. On unchanged entries every one of those is a no-op, so the
// cost is wall-clock (an SSH/kubectl round trip each), not disruption — but
// with a dozen games that's a minute of watching steps scroll by to add one
// port.
//
// ProxyCTL owns these entries, so cluster state matching the last snapshot
// is a safe default assumption. It is still an ASSUMPTION: a gateway
// deleted out-of-band won't be rebuilt by a fast apply. "Full reconcile"
// in the UI (?full=1) forces every entry through, which is the fix for
// exactly that.
//
// Always reconciled regardless: entries that are new, whose operator-facing
// fields changed (changedFields — deliberately ignores applier-managed
// TunnelIP/GatewayPubKey so a post-apply pubkey fill isn't mistaken for an
// edit), or that have no gateway public key yet, meaning they never
// finished an apply.
func (a *API) gatewayWorkSet(full bool) map[string]bool {
	if full {
		return nil
	}
	old, have := a.appliedSnap()
	if !have {
		return nil // never applied (or pre-snapshot install): do everything
	}
	...
	for _, e := range a.store.List() {
		o, ok := byID[e.ID]
		if !ok || e.GatewayPubKey == "" || len(changedFields(o, *e)) > 0 {
			work[e.ID] = true
		}
	}
	return work
...
func (a *API) createDNS(w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	action, rec, err := a.cf.UpsertA(ctx, body.Name, a.infra.DNSIP())
	if err != nil {
		a.writeJSON(w, http.StatusBadGateway, map[string]string{"error": err.Error()})
		return
	}
	a.writeJSON(w, http.StatusOK, map[string]any{
		"action": action, "record": rec, "ip": a.infra.DNSIP()})
}
...
func (a *API) getDroplet(w http.ResponseWriter, r *http.Request) {
	...
	if cfg != nil {
		out["ip"] = cfg.IP
		out["dnsIP"] = cfg.dnsIP()
		out["dnsIPExplicit"] = strings.TrimSpace(cfg.DNSPublicIP) != ""
		out["port"] = cfg.port()
		out["user"] = cfg.User
		if out["user"] == "" {
			out["user"] = "root"
		...
		out["wgTunnelIP"] = a.infra.DropletWGIP
		out["publicDNSIP"] = a.infra.DNSIP()
...
func (a *API) setDropletIPs(w http.ResponseWriter, r *http.Request) {
	cfg := a.droplet.Get()
	if cfg == nil {
		a.writeJSON(w, http.StatusBadRequest, map[string]string{"error": "Droplet not configured yet — finish setup first."})
		return
	}
	var body struct {
		IP    string `json:"ip"`
	...
	next := *cfg
	next.IP = strings.TrimSpace(body.IP)
	next.DNSPublicIP = strings.TrimSpace(body.DNSIP)
	if next.IP == "" {
		a.writeJSON(w, http.StatusBadRequest, map[string]string{"error": "The public endpoint IP is required."})
		return
	}
	if next.DNSPublicIP == "" {
		next.DNSPublicIP = next.IP
	}
	if err := a.droplet.Save(next); err != nil {
		a.writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
	a.refreshApplier()
...
func (a *API) restrictSSHToTunnel(w http.ResponseWriter, r *http.Request) {
	...
	if applyOK {
		...
		// refreshApplier (not just refreshInfra) so the live applier's
		// Droplet snapshot carries SSHTunnelOnly=true — that's what tells
		// runDropletSSH to stop falling back to the now-firewalled public
		// IP. refreshApplier calls refreshInfra internally, so the firewall
		// rebuild below still sees the updated infra.
		a.refreshApplier()
...
// droplet, the ban policy it's configured with (effectiveF2BPolicy —
// operator-tuned values over the fail2ban.go defaults, a single source
// of truth so this can never drift from what's actually applied), and
// the live banned-IP list enriched with best-effort geolocation.
func (a *API) fail2banStatus(w http.ResponseWriter, r *http.Request) {
	...
	pol := effectiveF2BPolicy(cfg.F2BPolicy)
	out := map[string]any{
		"maxRetry": pol.MaxRetry, "findTime": pol.FindTime, "banTime": pol.BanTime,
	}
	...
	// Daemon off (operator toggled it, or it crashed): every fail2ban-client
	// query below talks to the daemon's socket and just errors after burning
	// an SSH round-trip each — the UI sat on "Checking…" for their combined
	// timeouts. Skip straight to the log-derived history (a plain file read,
	// no daemon needed) so the "it's off" state renders fast and still shows
	// what happened before it was switched off.
	if !isActive {
		out["banned"] = []BannedIP{}
		out["recentAttempts"] = []IPAttempt{}
		hist := sa.sshDropletRaw("fail2ban: ban history", "",
			fmt.Sprintf("tail -n %d /var/log/fail2ban.log 2>/dev/null", f2bLogHistoryLines))
		out["banHistory"] = parseFail2banLogHistory(hist.Stdout)
		a.writeJSON(w, http.StatusOK, out)
		return
	}
	...
	att := sa.sshDropletRaw("fail2ban: recent attempts", "",
		"journalctl _COMM=sshd --no-pager --since "+f2bJournalSince(pol.FindTime)+" 2>/dev/null")
...
func (a *API) setupFail2ban(w http.ResponseWriter, r *http.Request) {
	...
	st := sa.sshDropletLong("fail2ban: install + configure", "",
		"bash -c "+shq(renderFail2banSetupScript(effectiveF2BPolicy(cfg.F2BPolicy))), 90*time.Second)
	a.writeJSON(w, http.StatusOK, map[string]any{
		"ok": st.OK, "stdout": st.Stdout, "stderr": st.Stderr,
	})
}

// setFail2banPolicy: POST /api/droplet/fail2ban/policy — store an
// operator-tuned ban policy and immediately re-render + re-apply the
// jail on the droplet so the stored intent and the running jail can't
// diverge. Values are validated against a tight charset BEFORE either
// step (they're interpolated into a root heredoc — see f2bTimeRe).
// Note the apply is a fail2ban restart: while the service flips over,
// its chains are briefly down, and bans re-assert from its own DB on
// start — a visible Unban/Ban pair per IP in the history, not a policy
// change.
func (a *API) setFail2banPolicy(w http.ResponseWriter, r *http.Request) {
	var body F2BPolicy
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		a.writeJSON(w, http.StatusBadRequest, map[string]string{"error": "invalid json"})
		return
	}
	pol := effectiveF2BPolicy(&body) // fill blanks with defaults, then validate the result
	if err := validateF2BPolicy(pol); err != nil {
		a.writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
	cfg := a.droplet.Get()
	...
	if err := a.droplet.SetF2BPolicy(&pol); err != nil {
		a.writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "saving policy: " + err.Error()})
		return
	}
	sa.Droplet = cfg
	st := sa.sshDropletLong("fail2ban: apply new policy", "",
		"bash -c "+shq(renderFail2banSetupScript(pol)), 90*time.Second)
	a.writeJSON(w, http.StatusOK, map[string]any{
		"ok": st.OK, "policy": pol, "stdout": st.Stdout, "stderr": st.Stderr,
	})
}

// setFail2banEnabled: POST /api/droplet/fail2ban/service — turn the
// whole service on or off. Enable is just the idempotent setup script
// (installs if missing, re-asserts config, enables + starts). Disable
// stops AND disables the unit so a reboot doesn't silently bring it
// back — while disabled, existing bans stop being enforced (their
// firewall chains come down with the service); they re-assert from
// fail2ban's own database the moment it's enabled again.
func (a *API) setFail2banEnabled(w http.ResponseWriter, r *http.Request) {
	var body struct {
	...
	var st Step
	if body.Enabled {
		st = sa.sshDropletLong("fail2ban: enable + start", "",
			"bash -c "+shq(renderFail2banSetupScript(effectiveF2BPolicy(cfg.F2BPolicy))), 90*time.Second)
	} else {
		st = sa.sshDroplet("fail2ban: stop + disable", "",
			"systemctl stop fail2ban 2>/dev/null; systemctl disable fail2ban 2>/dev/null; systemctl is-active fail2ban || true")
	}
	a.writeJSON(w, http.StatusOK, map[string]any{
		"ok": st.OK, "enabled": body.Enabled, "stdout": st.Stdout, "stderr": st.Stderr,
	})
}
...
func (a *API) setupControlTunnel(w http.ResponseWriter, r *http.Request) {
	...
	// Rebuild the live applier so the NEXT apply/delete/stats immediately
	// prefers the tunnel — without this the long-lived applier keeps the
	// process-start snapshot (ControlTunnelReady=false) and keeps SSHing
	// the public IP, which the very next lockdown step then firewalls off.
	a.refreshApplier()
	a.writeJSON(w, http.StatusOK, map[string]any{
		"ok": true, "pubKey": pub,
...
// ApplyScope limits what one apply touches. The two halves of ProxyCTL are
// independent — proxy entries are WireGuard gateways + droplet firewall
// rules, web apps are a Cloudflare Tunnel — but a single global apply meant
// pressing Apply on the Web Apps page also re-pushed every proxy entry
// (minutes of SSH, and a brief tunnel drop) for a change that couldn't
// possibly affect them. Each page now applies its own half.
type ApplyScope string

const (
	ScopeAll     ApplyScope = "all"     // both halves; the pre-scope behavior
	ScopeEntries ApplyScope = "entries" // WireGuard gateways + droplet + their DNS
	ScopeWebApps ApplyScope = "webapps" // Cloudflare Tunnel routes only
)

func (s ApplyScope) label() string {
	switch s {
	case ScopeEntries:
...
func (s ApplyScope) doesEntries() bool { return s == ScopeAll || s == ScopeEntries }
func (s ApplyScope) doesWebApps() bool { return s == ScopeAll || s == ScopeWebApps }

// applyScopeOf reads ?scope= (or {"scope":…}), defaulting to ScopeAll so an
// older UI build, a bookmarked call, or a script keeps its old behavior.
func applyScopeOf(r *http.Request) ApplyScope {
	v := strings.TrimSpace(r.URL.Query().Get("scope"))
	if v == "" && r.Body != nil {
		var body struct {
			Scope string `json:"scope"`
		}
		if json.NewDecoder(r.Body).Decode(&body) == nil {
			v = strings.TrimSpace(body.Scope)
		}
	}
	switch ApplyScope(v) {
	case ScopeEntries:
...
func (a *API) apply(w http.ResponseWriter, r *http.Request) {
	scope := applyScopeOf(r)
	// ?full=1 forces every gateway through phases 1-3 instead of only the
	// changed ones — the escape hatch for cluster state that drifted out
	// from under the snapshot (a gateway deleted by hand, a half-finished
	// apply). Nothing else about the apply differs.
	full := r.URL.Query().Get("full") == "1"
	work := a.gatewayWorkSet(full)
	...
	msg := "applying " + scope.label() + "…"
	if full {
		msg = "full reconcile of " + scope.label() + "…"
	}
	a.job = &ApplyJob{Running: true, StartedAt: time.Now().Unix(), Message: msg, Scope: string(scope)}
	a.jobMu.Unlock()
	a.saveJob()
	...
	go func() {
		// Starts OK so a web-apps-only apply isn't reported as failed just
		// because the droplet half never ran.
		res := ApplyResult{OK: true, Message: "applied " + scope.label()}
		if !scope.doesEntries() {
			// skip the droplet/gateway half entirely
		} else if sa, ok := a.applier.(SSHApplier); ok {
			// Always run against the LIVE droplet config, not whatever
			// snapshot the shared applier happens to hold — so a mid-session
			// migration (control tunnel set up / SSH locked to it) is honored
			// on the very next apply, including the teardown SSH that removes
			// a deleted entry's droplet rules. Without this a stale snapshot
			// SSHes the firewalled public IP and the removal fails.
			sa.Droplet = a.droplet.Get()
			...
			res = sa.ApplyPerGame(a.infra, a.store.List(), work, persist, onStep)
		} else {
		...
		// Entry-scoped: these A records point at the droplet, so they belong
		// with the proxy entries, not with the tunnel's CNAMEs.
		if scope.doesEntries() && res.OK && a.cf.Configured() {
			...
			for _, e := range a.store.List() {
				...
				action, rec, err := a.cf.UpsertA(ctx, e.Subdomain, a.infra.DNSIP())
				if err != nil {
		...
		if routes := a.webroutes.List(); scope.doesWebApps() && len(routes) > 0 {
...
// key), an active-connection count from /proc/net/nf_conntrack on the
// droplet (no extra tooling needed), and the droplet's WAN byte counters so
// the quota number can fold in the in-flight delta between sampler ticks.
// The UI polls this and derives a live rate from successive samples.
func (a *API) stats(w http.ResponseWriter, r *http.Request) {
	...
	// Same WAN read the sampler does (interface from the wizard config,
	// falling back to the default-route interface), appended after a __WAN__
	// marker so this 5s poll can fold the in-flight WAN delta into the
	// quota number without waiting for the next 10s sampler tick.
	iface := "eth0"
	if dc := a.droplet.Get(); dc != nil && dc.WANIface != "" {
		iface = dc.WANIface
	}
	st := sa.sshDroplet("stats", "",
		"wg show wg0 dump; echo '==CT=='; (conntrack -L 2>/dev/null || cat /proc/net/nf_conntrack 2>/dev/null || echo __NOCT__); echo __WAN__; IF='" + iface + "'; " +
		"[ -e \"/sys/class/net/$IF\" ] || IF=$(ip -o route show default | awk '{print $5; exit}'); " +
		"[ -e \"/sys/class/net/$IF/statistics/tx_bytes\" ] && cat /sys/class/net/$IF/statistics/rx_bytes /sys/class/net/$IF/statistics/tx_bytes")
	dump, rest, _ := strings.Cut(st.Stdout, "==CT==")
	ct, wanBlock := rest, ""
	if i := strings.Index(rest, "\n__WAN__"); i >= 0 {
		ct, wanBlock = rest[:i], rest[i+len("\n__WAN__"):]
	}
	...
	// WAN counters: the two numbers after the marker, if both actually read.
	var wanRx, wanTx int64
	haveWAN := false
	for n, f := range strings.Fields(wanBlock) {
	...
	for _, e := range a.store.List() {
		...
		m := map[string]any{"id": e.ID, "name": e.Name, "connections": conns,
			"rxBytes": 0, "txBytes": 0, "rxTotal": 0, "txTotal": 0, "lastHandshake": 0, "online": false}
		if p, ok := peers[e.GatewayPubKey]; ok && e.GatewayPubKey != "" {
			...
			// Persistent totals survive pod restarts / peer re-keys: the
			// sampler's accumulated counter plus this tick's in-flight delta.
			// rxBytes/txBytes above are the raw WireGuard counters (reset on
			// re-key); the *Total fields never roll back.
			if a.traffic != nil {
				m["rxTotal"], m["txTotal"] = a.traffic.entryLive(e.ID, rawBytes{rx: p.rx, tx: p.tx})
			}
		}
		out = append(out, m)
	}
	resp := map[string]any{"stats": out}
	if haveWAN {
		resp["wanRxTotal"], resp["wanTxTotal"] = a.traffic.WanLive(rawBytes{rx: wanRx, tx: wanTx})
	}
	a.writeJSON(w, http.StatusOK, resp)
}

// entryUptimeHistory returns GET /api/entries/{id}/uptime — ~1h of
// WireGuard-tunnel reachability samples for the Tunnels table's sparkline.
func (a *API) entryUptimeHistory(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if _, ok := a.store.Get(id); !ok {
		a.writeJSON(w, http.StatusNotFound, map[string]string{"error": "no such entry"})
		return
	}
	h := UptimeHistoryForEntry(id)
	a.writeJSON(w, http.StatusOK, map[string]any{"available": len(h) > 0, "history": h})
}

// webRouteUptimeHistory returns GET /api/webroutes/{id}/uptime — ~1h of
// HTTPS reachability samples for the Web Apps table's sparkline.
func (a *API) webRouteUptimeHistory(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if _, ok := a.webroutes.Get(id); !ok {
		a.writeJSON(w, http.StatusNotFound, map[string]string{"error": "no such route"})
		return
	}
	h := UptimeHistoryForWebRoute(id)
	a.writeJSON(w, http.StatusOK, map[string]any{"available": len(h) > 0, "history": h})
}
...
// writeSaveError turns a store.Put failure into JSON. A port conflict
// additionally carries its structure (contested port, holder, nearest
// free port) so the UI can offer "publish on N instead → delivered to
// the original port in-cluster" as one click instead of a dead end.
func (a *API) writeSaveError(w http.ResponseWriter, status int, err error) {
	var pc *PortConflictError
	if errors.As(err, &pc) {
		a.writeJSON(w, http.StatusConflict, map[string]any{
			"error": pc.Error(),
			"conflict": map[string]any{
				"port": pc.Port, "proto": pc.Proto,
				"holder": pc.Holder, "suggested": pc.Suggested,
			},
		})
		return
	}
...
func (a *API) create(w http.ResponseWriter, r *http.Request) {
	...
	e.ID = newID()
	if err := a.store.Put(e); err != nil {
		a.writeSaveError(w, http.StatusBadRequest, err)
		return
	}
	a.respondSaved(w, e)
}
...
func (a *API) update(w http.ResponseWriter, r *http.Request) {
	...
	e.ID = id
	// Carry forward the fields the SERVER owns when the request doesn't
	// carry them. An edit form sends the operator-visible fields (name,
	// ports, target, …) and knows nothing about the data plane — without
	// this, saving a one-character name change would blank TunnelIP and
	// GatewayPubKey, which reallocates the pinned tunnel address and forces
	// every gateway to re-key: the tunnel drops and the droplet peer has to
	// be re-registered, for an edit that should have changed a label.
	if e.TunnelIP == "" {
		e.TunnelIP = cur.TunnelIP
	}
	if e.GatewayPubKey == "" {
		e.GatewayPubKey = cur.GatewayPubKey
	}
	if e.Mode == "" {
		e.Mode = cur.Mode
	}
...
func (a *API) toggle(w http.ResponseWriter, r *http.Request) {
	...
	e.Enabled = !e.Enabled
	if err := a.store.Put(e); err != nil {
		// Re-enabling can hit a port conflict created while this entry sat
		// disabled — same structured response as create/update.
		a.writeSaveError(w, http.StatusInternalServerError, err)
		return
	}
	a.respondSaved(w, e)
}
