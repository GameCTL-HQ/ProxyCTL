...
// containerImages returns every `image:` value in a single-document manifest.
// In a Pod spec `image:` only ever appears under
// containers/initContainers/ephemeralContainers, so this is a complete,
// side-effect-free way to enumerate what the doc would pull without a full
// YAML parser (consistent with this file's line-based parsing elsewhere).
// It matches `image:` as a mapping key in both block style
// (`  image: foo`) and flow style (`  - { name: x, image: foo }`) so a
// manifest can't hide a container image by reformatting.
func containerImages(doc string) []string {
	var imgs []string
	for _, ln := range strings.Split(doc, "\n") {
		for {
			idx := strings.Index(ln, "image:")
			if idx < 0 {
				break
			}
			// `image:` must be a standalone key, not a substring of a longer
			// token (e.g. `someimage:`): it must sit at line start or follow
			// whitespace / a flow separator.
			if idx > 0 {
				switch prev := ln[idx-1]; prev {
				case ' ', '\t', ',', '{':
				default:
					ln = ln[idx+len("image:"):]
					continue
			...
			rest := strings.TrimLeft(ln[idx+len("image:"):], " \t")
			// Value runs to end-of-line, or (flow style) the next , or }.
			end := len(rest)
			...
			if c := strings.Index(val, " #"); c >= 0 { // strip inline comment
...
// verifyDeploymentImages fail-closes the self-apply path. The release
// manifest is fetched from the public repo and applied with ProxyCTL's own
// in-cluster credentials, so a compromised release is the highest-impact
// supply-chain payload available here — its most useful trick would be to add
// a malicious sidecar/initContainer image to the Deployment. Every container
// image in the self-applied Deployment must therefore either be the exact
// image this update is rolling to (newImage) or pinned by an @sha256 digest;
// anything referencing a floating tag (e.g. evil.io/payload:latest or :v1) is
// refused. The digest-pinned allowance (rather than pinning to one hard-coded
// first-party digest) is deliberate: a future release that bumps the
// control-wg sidecar image must still be applicable, and digest-pinning is
// forward-compatible while a floating tag is not.
func verifyDeploymentImages(docs []string, newImage string) error {
	for _, doc := range docs {
		if docKind(doc) != "Deployment" {
			continue
		}
		for _, img := range containerImages(doc) {
			if img == newImage || strings.Contains(img, "@sha256:") {
				continue
			}
			return fmt.Errorf("refusing to apply release manifest: Deployment references non-pinned image %q (every container image must be %s or pinned by @sha256:)", img, newImage)
		}
	}
	return nil
}
...
type releaseApplyResult struct {
	...
	// manifestSHA256 is the hex SHA-256 of the raw release manifest fetched
	// for the target tag — the exact bytes of k8s/proxyctl.yaml as shipped at
	// that tag. Surfaced so the operator can audit/compare what updateApply
	// is about to (or did) apply against the file in the repo at that tag.
...
func (a *API) applyRelease(ctx context.Context, containerName, newImage, curTag, latest string) (releaseApplyResult, error) {
	...
	targetSHA := sha256Hex([]byte(target))
	...
	// Fail closed before touching the cluster: a fetched manifest that would
	// pull a floating-tag container image is rejected, not applied.
	if err := verifyDeploymentImages(applyDocs, newImage); err != nil {
		return releaseApplyResult{}, err
	}
