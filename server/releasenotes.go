...
//   - the newest RELEASED entry (SHA/dev builds between tags).
//
...
func relForVersion(v string) (rel relEntry, found bool, err error) {
	...
	// Never fall back to a staging "unreleased" entry: between tags the
	// operator runs a SHA/dev build, and showing them an "Unreleased"
	// section — usually empty — hid what actually shipped most recently.
	for i := range all {
		r := &all[i]
