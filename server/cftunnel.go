...
func (c *CF) accountID(ctx context.Context) (string, error) {
	...
	// 2. Fallback: every zone object carries its owning account's id. A
	//    token can span SEVERAL accounts (e.g. Zone DNS Edit on zones in
	//    two accounts but Tunnel Edit only on one), so collect the
	//    distinct account ids and probe each with a cheap cfd_tunnel
	//    list — pick the first one the token can actually read.
	seen := map[string]bool{}
	var accts []string
	if raw, err := c.do(ctx, "GET", "/zones?per_page=100", nil); err == nil {
		var zs []struct {
			Account struct {
				ID string `json:"id"`
			} `json:"account"`
		}
		if json.Unmarshal(raw, &zs) == nil {
			for _, z := range zs {
				if z.Account.ID != "" && !seen[z.Account.ID] {
					seen[z.Account.ID] = true
					accts = append(accts, z.Account.ID)
				}
			}
		}
	}
	for _, a := range accts {
		if _, err := c.do(ctx, "GET", "/accounts/"+a+"/cfd_tunnel?per_page=1", nil); err == nil {
			return setAcct(a)
		}
	}
