...
// The sysctl init container historically ran privileged:true (full sandbox
// escape) to set forwarding/conntrack sysctls — but those are net.* namespaced
// sysctls writable with CAP_NET_ADMIN alone (the wireguard container below
// already does more with just NET_ADMIN). Guard against the regression.
func TestRenderGateway_SysctlInitNotPrivileged(t *testing.T) {
	for _, in := range []Infra{shareInfra(), DefaultInfra()} {
		got := renderGatewayManifest(in, gwEntry())
		if strings.Contains(got, "privileged: true") {
			t.Errorf("sysctl init container must not run privileged:\n%s", got)
		}
		if !strings.Contains(got, `allowPrivilegeEscalation: false, capabilities: { drop: ["ALL"], add: ["NET_ADMIN"] }`) {
			t.Errorf("sysctl init container must drop all caps and add only NET_ADMIN:\n%s", got)
		}
	}
}
