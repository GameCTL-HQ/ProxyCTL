...
// The rendered jail config must actually carry the constants the status
...
func TestRenderFail2banSetupScript_MatchesPolicyConstants(t *testing.T) {
	script := renderFail2banSetupScript()
	...
	// No escalation, no second jail — a single permanent-from-first-offense
	// tier is the whole policy now (see the const block's reasoning).
