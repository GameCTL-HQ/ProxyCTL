...
var existingDataProbeRe = regexp.MustCompile(`PROXYCTL_EXISTING app_files=(\d+) key_dirs=(\d+)`)

// parseExistingDataProbe reads the probe pod's own stdout (see the "sh -c"
// command in testStorageShare) for the PROXYCTL_EXISTING marker line it
// prints after the normal write/read check. Missing/unparseable output —
// an older probe image, say — is treated as "found nothing" rather than an
// error, since this check is advisory on top of the pass/fail mount test.
func parseExistingDataProbe(logs string) (appFiles, keyDirs int) {
	m := existingDataProbeRe.FindStringSubmatch(logs)
	if m == nil {
		return 0, 0
	}
	appFiles, _ = strconv.Atoi(m[1])
	keyDirs, _ = strconv.Atoi(m[2])
	return
}
...
spec:
  ...
  containers:
    - name: probe
      ...
      # After the write/read check, also look for data a PRIOR install left
      # in this exact directory — app/ is where adopt mounts ProxyCTL's own
      # /data (subPath "app"), Keys/ is where gateway WireGuard keys land.
      # Reported back over stdout (pod logs) so a re-install pointed at an
      # old share can tell the operator what's already there instead of
      # silently reusing it — see testStorageShare's log-parsing below.
      command: ["sh", "-c", "echo proxyctl-probe > /probe/.proxyctl-probe && cat /probe/.proxyctl-probe && rm -f /probe/.proxyctl-probe; APPFILES=$(find /probe/app -maxdepth 1 -type f 2>/dev/null | wc -l); KEYDIRS=$(find /probe/Keys -mindepth 1 -maxdepth 1 -type d 2>/dev/null | wc -l); echo PROXYCTL_EXISTING app_files=$APPFILES key_dirs=$KEYDIRS"]
      volumeMounts:
...
`, podName, a.selfNS, image, subPathLine, srv, mountPath)
	...
	for time.Now().Before(deadline) {
		...
		case "Succeeded":
			logs, _ := runKubectlKeys("", "-n", a.selfNS, "logs", podName)
			...
			// Surfaced only when there's actually something to say — a fresh
			// share has zero of both, and staying silent there matches every
			// other install's experience instead of a permanently-present
			// "nothing found" line.
			if appFiles > 0 || keyDirs > 0 {
				resp["existingData"] = map[string]any{"appFiles": appFiles, "keyDirs": keyDirs}
			}
