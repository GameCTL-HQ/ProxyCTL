...
spec:
  ...
  volumes:
    - name: probe
      nfs: { server: %s, path: %s }
`, podName, a.selfNS, image, srv, exp)
...
`, a.selfNS, sc)
	...
	ops, err := a.dataVolumePatchOps(r.Context(), volume, subPath, envSrv, envExp)
	if err != nil {
...
// sets/clears the mount subPath, and sets/clears the share env vars — with
// indices read from the LIVE spec, since a patch by position must match it.
func (a *API) dataVolumePatchOps(ctx context.Context, volumeJSON, subPath, envSrv, envExp string) (string, error) {
	out, err := exec.CommandContext(ctx, a.kubectl, "-n", a.selfNS,
		"get", "deploy/"+a.selfDeploy, "-o", "json").CombinedOutput()
	if err != nil {
