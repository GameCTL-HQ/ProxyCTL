...
func (a *API) getKeysConfig(w http.ResponseWriter, r *http.Request) {
	...
	out := map[string]any{
		...
		// thing that knows where keys really land.
		"locationHint": keysLocationHint(base),
		// The export root on its own, so the UI can compose a live preview as
		// the operator types a folder. Empty when the backing store isn't an
		// nfs-subdir export (or isn't discoverable) — the UI then says what it
		// actually knows instead of inventing a root.
		"nfsRoot":          sf.NFSRoot(),
		"nfsServer":        sf.NFSServer,
