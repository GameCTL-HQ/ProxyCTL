...
func (a *API) setKeysConfig(w http.ResponseWriter, r *http.Request) {
	...
	// Share mode: make sure the static PV + shared claim exist now so a
	// subsequent gateway apply binds without waiting. When the PV already
	// exists (install-time share, or an earlier save) only the namespaced
	// PVC is ensured — touching an existing PV needs cluster-scoped RBAC
	// the ServiceAccount deliberately doesn't have.
	if srv, exp, ok := a.keys.Share(); ok {
		volName := keysShareVolName(srv, exp)
		manifest := renderKeysSharePV(a.infra.K8sNamespace, srv, exp)
		if _, err := runKubectlKeys("", "get", "pv", volName); err == nil {
			manifest = renderKeysSharePVC(a.infra.K8sNamespace, srv, exp)
		}
		if out, err := runKubectlKeys(manifest, "apply", "-f", "-"); err != nil {
			a.writeJSON(w, http.StatusBadGateway, map[string]any{
				"error": "saved the share, but creating its PV/PVC failed: " + strings.TrimSpace(out),
			})
			return
		}
