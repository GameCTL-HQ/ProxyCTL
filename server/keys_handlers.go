...
func (a *API) getKeysConfig(w http.ResponseWriter, r *http.Request) {
	...
	out := map[string]any{
		...
		// invented here.
		"discoveredNfsServer": sf.NFSServer,
		"discoveredNfsExport": sf.NFSRoot(),
...
func (a *API) setKeysConfig(w http.ResponseWriter, r *http.Request) {
	...
	// Share mode: create the static PV + shared claim now so a subsequent
	// gateway apply binds without waiting. Idempotent — apply, not create.
	if srv, exp, ok := a.keys.Share(); ok {
		volName := keysShareVolName(srv, exp)
		if out, err := runKubectlKeys(renderKeysSharePV(a.infra.K8sNamespace, srv, exp),
			"apply", "-f", "-"); err != nil {
