...
type DropletConfig struct {
	...
	// WANIface is the droplet's DETECTED public egress interface
	// (`ip route get 1.1.1.1` → dev), captured at bootstrap and refreshed
	// on every Apply. The NAT/DNAT rules bind to it — a hardcoded "eth0"
	// is only right on providers that name it that (DigitalOcean yes;
	// Hetzner/AWS-style ens3/enX0 no), and a wrong iface silently keeps
	// public traffic out of the PROXYCTL chains.
...
type DropletStore interface {
	...
	// SetWANIface records the droplet's detected public egress interface.
	SetWANIface(iface string) error
...
func (s *fileDropletStore) SetWANIface(iface string) error {
