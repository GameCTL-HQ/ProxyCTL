...
type DropletConfig struct {
	...
	DNSPublicIP    string `json:"dnsPublicIP"`    // player/DNS-facing IPv4; Cloudflare A records point here
...
func (d *DropletConfig) dnsIP() string {
	if ip := strings.TrimSpace(d.DNSPublicIP); ip != "" {
		return ip
	}
...
func (cfg DropletConfig) validate() error {
	...
	if ip := strings.TrimSpace(cfg.DNSPublicIP); ip != "" && net.ParseIP(ip) == nil {
		return errors.New("dns public IP must be a valid IPv4 / IPv6 address")
	}
...
func (s *fileDropletStore) Save(cfg DropletConfig) error {
	if strings.TrimSpace(cfg.DNSPublicIP) == "" {
		cfg.DNSPublicIP = strings.TrimSpace(cfg.IP)
	}
	...
	if prev := s.cfg; prev != nil {
		cfg.PublicKey = prev.PublicKey
		if cfg.DNSPublicIP == "" {
			cfg.DNSPublicIP = prev.DNSPublicIP
		}
		cfg.WGPublicKey = prev.WGPublicKey
		cfg.Bootstrapped = prev.Bootstrapped
		cfg.BootstrappedAt = prev.BootstrappedAt
		cfg.WANIface = prev.WANIface
		cfg.WANIfaceManual = prev.WANIfaceManual
		cfg.WANIfaces = prev.WANIfaces
		cfg.SSHLockedDown = prev.SSHLockedDown
		cfg.SSHAllowedIPs = prev.SSHAllowedIPs
		cfg.SSHLockedAt = prev.SSHLockedAt
		cfg.SSHTunnelOnly = prev.SSHTunnelOnly
		cfg.SSHFirewallGate = prev.SSHFirewallGate
		cfg.ControlTunnelReady = prev.ControlTunnelReady
		cfg.ControlTunnelAt = prev.ControlTunnelAt
		cfg.ControlTunnelNudgeDismissed = prev.ControlTunnelNudgeDismissed
		cfg.PersonalAccessPeers = prev.PersonalAccessPeers
		cfg.F2BPolicy = prev.F2BPolicy
	}
