...
export default function WebRoutes({ domains, onRoutesChange, onChanged }) {
  ...
  // True once the first /api/webroutes fetch has completed — until then the
  // table would otherwise flash "No web routes yet" on every page refresh.
          ...
          <tbody>
            {routes.length === 0 && (
              <tr><td colSpan="5" style={{ color: 'var(--muted)' }}>
                {routesLoaded
                  ? 'No web routes yet — add one below.'
                  : <><span className="spin" />loading web routes…</>}
              </td></tr>
            )}
            {routes.map(rt => (
              <tr key={rt.id}>
