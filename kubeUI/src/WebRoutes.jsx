...
export default function WebRoutes({ domains }) {
  ...
  return (
    <>
      <p className="sub">Public hostname &rarr; in-cluster Service, through a
        Cloudflare Tunnel. Cloudflare's edge terminates TLS and runs WAF / DDoS
        protection — no droplet, no public IP, no certs. (For raw game ports
        use the Proxy Entries tab.)</p>
      ...
      <div className="card">
        <table>
          <thead>
            <tr><th>Status</th><th>Hostname</th><th>&rarr; Backend service</th><th></th></tr>
          </thead>
          <tbody>
            {routes.length === 0 && (
              <tr><td colSpan="4" style={{ color: 'var(--muted)' }}>No web routes yet — add one below.</td></tr>
            )}
            {routes.map(rt => (
              <tr key={rt.id}>
                <td><span className={'stat ' + (rt.enabled ? 'down' : 'dis')}>
                  <span className="d" /><span className="t">{rt.enabled ? 'enabled' : 'disabled'}</span></span></td>
                <td><strong>{rt.hostname}</strong></td>
