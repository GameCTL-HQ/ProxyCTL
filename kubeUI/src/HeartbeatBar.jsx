    ...
    <div>
      ...
      <div style={{ width: stripWidth }}>
        <div style={{ display: 'flex', gap, alignItems: 'stretch', height: compact ? 16 : 32 }}>
          {buckets.map((b, i) => (
            <div className="hb-seg-wrap" key={i}>
              <div
                className="hb-seg"
                style={{ background: colorFor(b.state), opacity: b.state === 'empty' ? 0.5 : 1 }}
              />
              <span className={`hb-tip ${b.state}`}>
                ...
                <span className="s">
                  ...
                  {b.state === 'empty'
                    ? 'no samples'
                    : b.state === 'up'
                      ? `up · ${b.samples.length} check${b.samples.length === 1 ? '' : 's'}`
                      : `down · ${b.down}/${b.samples.length} failed`}
                </span>
              </span>
            </div>
          ))}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginTop: 3 }}>
          {axisTimes.map((t, i) => (
            <span key={i} style={{ fontSize: compact ? 9 : 11, color: 'var(--muted)', whiteSpace: 'nowrap' }}>{fmtAxis(t)}</span>
          ))}
        </div>
      </div>
    </div>
