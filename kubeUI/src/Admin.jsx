...
import { cleanFolder } from './paths.js'
...
// Tab persistence. Validated against this list on read so a stale or
// hand-edited value can't land the app on a tab that no longer exists.
const TAB_KEY = 'proxyctl_tab'
const TABS = ['games', 'web', 'security', 'monitoring', 'config']
...
export default function Admin({ onSignOut, onOpenSetup }) {
  ...
  // True once the first /api/entries fetch has completed — until then the
  // table would otherwise flash "No entries yet" on every page refresh.
  ...
  const [quota, setQuota] = useState(null)     // WAN egress quota: {txTotal, quotaBytes, nearPct, level, resetDay, lastResetAt}
  const [wanLive, setWanLive] = useState(null) // {rx, tx} — live WAN counters from /api/stats, 5s while the page is open
  const [quotaBusy, setQuotaBusy] = useState(false)
  const isNarrow = useIsNarrow()
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)

  const [pickerOpen, setPickerOpen] = useState(false)
  // Tab persists across reloads so a refresh (or a re-login) puts you back
  // where you were instead of bouncing to a fixed landing tab. First-ever
  // visit opens Proxy Entries — the thing ProxyCTL is for — rather than
  // Configuration, which is setup you touch once.
  ...
  const [ipForm, setIPForm] = useState({ ip: '', dnsIP: '' })
  ...
  // Non-null while the form is editing an existing entry rather than
  // adding one — see startEdit.
  ...
  // Refresh the unapplied-changes state after a change made by a CHILD
  // panel (web routes). Must be referentially stable: WebRoutes rebuilds
  // its loader whenever a prop identity changes, and an inline arrow here
  // would turn that into a fetch loop.
  const refreshPending = useCallback(() => { loadEntries() }, [loadEntries])
  ...
  useEffect(() => {
    setIPForm({
      ip: droplet?.ip || '',
      dnsIP: droplet?.dnsIPExplicit ? (droplet?.dnsIP || '') : (droplet?.ip || ''),
    })
  }, [droplet?.ip, droplet?.dnsIP, droplet?.dnsIPExplicit])
  ...
  const pollStats = useCallback(async () => {
    ...
    setStats(next)
    // The same SSH read also carries the droplet's WAN byte counters; fold
    // in the in-flight delta server-side so the quota number moves at this
    // poll's rate rather than the sampler's 10s tick.
    if (d.wanRxTotal != null || d.wanTxTotal != null) {
      setWanLive({ rx: d.wanRxTotal || 0, tx: d.wanTxTotal || 0 })
    }
  }, [])

  const pollQuota = useCallback(async () => {
    const { ok, data: d } = await apiJSON('/api/traffic/quota')
    if (ok) setQuota(d)
  }, [])

  const resetQuota = useCallback(async () => {
    setQuotaBusy(true)
    const { ok, data: d } = await postJSON('/api/traffic/quota/reset', {})
    setQuotaBusy(false)
    if (!ok) { alert(d?.error || 'Failed to reset traffic quota') ; return }
    pollQuota()
  }, [pollQuota])
  ...
  async function saveDropletIPs() {
    if (!ipForm.ip.trim()) {
      toast('Public endpoint IP is required.', true)
      return
    }
    setIPBusy(true)
    setIPMsg('')
    try {
      const { ok, data } = await postJSON('/api/droplet/ips', {
        ip: ipForm.ip.trim(),
        dnsIP: (ipForm.dnsIP.trim() || ipForm.ip.trim()),
      })
      if (!ok) {
        setIPMsg(data?.error || 'Save failed')
        return
      }
      setDroplet(data)
      setIPMsg('Saved')
      toast('✓ IP settings saved')
    } finally {
      setIPBusy(false)
    }
  }
  ...
  // === Initial boot =========================================================
  useEffect(() => {
    loadEntries(); loadDomains(); cfStatus(); loadDroplet(); pollStats(); pollQuota(); pollApply()
    const t = setInterval(() => { pollStats(); pollQuota() }, 5000)
  ...
  // Serialize an entry back into the add-form's shape so the same form can
  // edit it. Ports round-trip through the "27017->27015:udp" text form the
  // form already parses.
  function portsToText(ports) {
    return (ports || []).map(p => {
      const pub = p.targetPort && p.targetPort !== p.port ? `${p.port}->${p.targetPort}` : String(p.port)
      return p.proto && p.proto !== 'both' ? `${pub}:${p.proto}` : `${pub}:both`
    }).join(', ')
  }

  // startEdit loads an existing entry into the form. Editing used to mean
  // delete + re-add, which tore down the entry's gateway and re-keyed its
  // tunnel just to change a port or a label.
  function startEdit(e) {
    ...
    setForm({
      ...
      ports: portsToText(e.ports), targetIP: e.targetIP || '',
      service: e.service || '', enabled: e.enabled !== false,
      domain: (e.subdomain || '').split('.').slice(1).join('.'),
    })
  ...
  async function saveEntry(portsStr) {
    ...
    // PUT keeps the entry's id — and with it its pinned tunnel IP and
    // gateway key, which the server carries forward.
    const { ok, data } = editingId
      ? await putJSON(`/api/entries/${editingId}`, body)
      : await postJSON('/api/entries', body)
    if (!ok) {
  ...
  // scope defaults to the proxy entries this page owns — pushing the
  // Cloudflare Tunnel's web apps from here would reconcile a half the
  // operator isn't looking at. The Web Apps tab applies its own.
  async function applyNow(scope = 'entries', full = false) {
    const r = await apiFetch(
      `/api/apply?scope=${encodeURIComponent(scope)}${full ? '&full=1' : ''}`,
      { method: 'POST' })
  ...
  const showApplyControls = tab === 'games' || tab === 'web'
  // The Proxy Entries and Web Apps tabs share this Apply card, but they are
  // independent halves — WireGuard gateways + droplet rules vs. a Cloudflare
  // Tunnel. Apply pushes the half you're looking at, nothing else.
  const applyScope = tab === 'web' ? 'webapps' : 'entries'
  const applyScopeLabel = tab === 'web' ? 'web apps' : 'proxy entries'
  ...
  const totalStats = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      rx += (+(s.rxTotal ?? s.rxBytes) || 0)
      tx += (+(s.txTotal ?? s.txBytes) || 0)
    }
    ...
    <>
      ...
      {showApplyControls && (
        <div id="actionbar" className={'actionbar' + (data.pending ? ' on' : '')}>
          ...
          <button className="primary" onClick={() => applyNow(applyScope)}>Apply now</button>
        </div>
      )}
      ...
      <main>
        <h2 className="section">{
          ...
          {tab === 'config' && <span className="badge" style={{ marginLeft: 10 }}>network · dns</span>}
        ...
        <div className="row" style={{ gap: 8, margin: '0 0 18px' }}>
          {[
            ...
            ['config', 'Configuration', 'network IPs · DNS'],
          ].map(([k, lab, sub]) => (
            <button key={k}
            ...
            <div className="card">
              ...
              {!droplet?.configured ? (
                <p className="hint" style={{ margin: 0 }}>
                  Finish the droplet setup first, then manage the live addresses here.
                </p>
              ) : (
                <>
                  <p className="hint" style={{ margin: '0 0 14px' }}>
                    These are three different addresses. The first is the public IP ProxyCTL and WireGuard dial. The second is the public IP Cloudflare DNS should hand to external users. The third is the inside-the-tunnel `10.x` peer IP after WireGuard connects.
                  </p>
                  <div className="pair">
                    <label>
                      <span style={{ color: 'var(--link)', fontWeight: 700 }}>● Public endpoint IP</span>
                      <input
                        value={ipForm.ip}
                        onChange={e => setIPForm(v => ({ ...v, ip: e.target.value }))}
                        placeholder="e.g. 167.99.150.10"
                      />
                      <span className="hint"><strong style={{ color: 'var(--link)' }}>Used for:</strong> ProxyCTL SSH, droplet management, and the initial WireGuard endpoint.</span>
                    </label>
                    <label>
                      <span style={{ color: 'var(--ok-tx)', fontWeight: 700 }}>● Cloudflare bind IP</span>
                      <input
                        value={ipForm.dnsIP}
                        onChange={e => setIPForm(v => ({ ...v, dnsIP: e.target.value }))}
                        placeholder="e.g. 203.0.113.25"
                      />
                      <span className="hint">
                        {droplet?.dnsIPExplicit
                          ? <><strong style={{ color: 'var(--ok-tx)' }}>Used for:</strong> Cloudflare grey-cloud A records and the public DNS guide external users follow.</>
                          : <><strong style={{ color: 'var(--ok-tx)' }}>Used for:</strong> Cloudflare grey-cloud A records and the public DNS guide external users follow. Until you set this, DNS falls back to the public endpoint IP.</>}
                      </span>
                    </label>
                  </div>
                  <div className="pair" style={{ marginTop: 12 }}>
                    <label>
                      <span style={{ color: 'var(--warn)', fontWeight: 700 }}>● WireGuard tunnel IP</span>
                      <input
                        value={droplet?.wgTunnelIP || ''}
                        readOnly
                        placeholder="10.8.0.1"
                      />
                      <span className="hint"><strong style={{ color: 'var(--warn)' }}>Used for:</strong> the inside-the-tunnel peer address only. Not Cloudflare. Not the public endpoint.</span>
                    </label>
                  </div>
                  <div className="row" style={{ marginTop: 12 }}>
                    <button className="sm primary" disabled={ipBusy} onClick={saveDropletIPs}>
                      {ipBusy ? 'Saving…' : 'Save IPs'}
                    </button>
                    {ipMsg && <span className={ipMsg === 'Saved' ? 'ok' : 'bad'}>{ipMsg}</span>}
                  </div>
                </>
              )}
            </div>
        ...
        {(quota?.quotaBytes > 0 || totalStats.any) && (() => {
          const hasQuota = quota && quota.quotaBytes > 0
          // Live WAN number from the 5s stats poll (persisted total + in-flight
          // delta); fall back to the quota endpoint's last persisted tick.
          const used = wanLive ? wanLive.tx : (hasQuota ? quota.txTotal || 0 : 0)
          const cap = hasQuota ? quota.quotaBytes : 0
          const pct = hasQuota ? Math.min(100, (used / cap) * 100) : 0
          const full = hasQuota ? Math.round(used / cap * 100) : 0
          const barColor = quota.level >= 2 ? 'var(--err)' : quota.level >= 1 ? 'var(--warn)' : 'var(--ok)'
          const levelTxt = quota.level >= 2 ? 'over quota' : quota.level >= 1 ? 'near quota' : 'within quota'
          return (
            <div style={{
              ...
              {hasQuota && (<>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
                  <strong>WAN egress</strong>
                  <span style={{ color: 'var(--muted)', fontSize: 11 }}>· what DigitalOcean bills — all droplet outbound</span>
                  <span style={{ color: 'var(--ok-tx)' }}>↑ {fmtBytes(used)}</span>
                  <span style={{ color: 'var(--muted)' }}>/ {fmtBytes(cap)}</span>
                  <span style={{ color: barColor, fontWeight: 700 }}>{full}%</span>
                  <span style={{ border: `1px solid ${barColor}`, color: barColor, borderRadius: 4, padding: '1px 7px', fontSize: 11 }}>{levelTxt}</span>
                  <button className="sm" style={{ marginLeft: 'auto' }} disabled={quotaBusy} onClick={resetQuota}>{quotaBusy ? 'Resetting…' : 'Reset counters'}</button>
                </div>
              ...
              {totalStats.any && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
                  <strong>Through proxy entries</strong>
                  <span style={{ color: 'var(--muted)', fontSize: 11 }}>· WireGuard traffic only</span>
              ...
              <div style={{ color: 'var(--muted)', fontSize: 11, marginTop: 4 }}>
                {hasQuota && <>Outbound only — inbound is free on DO.</>}
                {hasQuota && quota.nearPct > 0 && <> Discord alerts at {quota.nearPct}%. </>}
                {hasQuota && (quota.resetDay > 0 ? <>Auto-resets monthly on day {quota.resetDay}.</> : <>No auto-reset (manual only).</>)}
                {hasQuota && quota.lastResetAt > 0 && <> Last reset {new Date(quota.lastResetAt * 1000).toLocaleString()}.</>}
                <>{(hasQuota || totalStats.any) && <> Numbers refresh every 5s while this page is open.</>}</>
              </div>
            </div>
          )
        })()}
              ...
              {rows.length === 0 && removed.length === 0 && (
                <tr><td colSpan="8" style={{ color: 'var(--muted)' }}>
                  {entriesLoaded
                    ...
                    : <><span className="spin" />loading entries…</>}
                </td></tr>
              )}
              {rows.map(r => <EntryRow key={r.entry.id} r={r} stats={stats[r.entry.id]} onToggle={toggleEntry} onRebind={rebindEntry} onDelete={deleteEntry} onEdit={startEdit} />)}
              {removed.map(rm => (
        ...
        {/* Add / edit entry */}
        <div className="card" id="entryForm">
          <h2>{editingId ? 'Edit tunnel entry' : 'Add tunnel entry'}</h2>
          {editingId && (
            <p className="hint" style={{ margin: '0 0 12px' }}>
              Editing an existing entry — it keeps its tunnel IP and gateway key, so saving
              doesn't re-key the tunnel. Changes go live on the next <strong>Apply</strong>.
            </p>
          )}
        ...
        {tab === 'web' && <WebRoutes domains={domains} onRoutesChange={setWebRoutes} onChanged={refreshPending} />}
        ...
        {showApplyControls && (
          <div className="card" id="applyCard">
            ...
            {tab === 'web' ? (
              <p className="hint" style={{ margin: '0 0 14px' }}>
                <strong>Apply now</strong> publishes your web apps through the Cloudflare Tunnel: it
                ensures the tunnel exists, (re)deploys the <span className="mono">cloudflared</span>
                {' '}connector, pushes the ingress rules for every enabled hostname, and creates or
                updates a proxied CNAME per hostname. Proxy entries and the droplet are untouched.
              </p>
            ) : (
              <>
                <p className="hint" style={{ margin: '0 0 10px' }}>
                  <strong>Apply now</strong> pushes your proxy entries live. It writes the droplet's
                  WireGuard config and rebuilds its firewall rules for the full peer set, creates or
                  updates the in-cluster gateway for each entry you added or changed, and — if
                  Cloudflare is configured — upserts each entry's DNS record. Entries you didn't touch
                  keep running: their gateways are left alone rather than re-pushed one at a time.
                  Web apps are untouched.
                </p>
                <p className="hint" style={{ margin: '0 0 14px' }}>
                  <strong>Full reconcile</strong> does the same push, but re-creates <em>every</em>
                  {' '}gateway instead of only the changed ones. Use it when the cluster no longer
                  matches what ProxyCTL applied:
                </p>
                <ul className="hint" style={{ margin: '-8px 0 14px 18px', padding: 0 }}>
                  <li>a gateway pod (or its Secret/PVC) was deleted or edited with <span className="mono">kubectl</span></li>
                  <li>an apply failed partway through, or the pod restarted mid-apply</li>
                  <li>one entry's tunnel is dead while the rest work, and Apply now reports nothing to do</li>
                </ul>
                <p className="hint" style={{ margin: '0 0 14px' }}>
                  It runs the identical reconcile with the skip logic off — nothing destructive, just
                  slower. Anything Apply now skips is named in the output below.
                </p>
              </>
            )}
            <div className={'pending' + (data.pending ? ' on' : '')}>
              <span className="dotp" /><span>Unapplied changes — your {applyScopeLabel} differ from what's live. Click <strong>Apply now</strong> to push them.</span>
            </div>
            <div className="row">
              <button className="primary" disabled={!!applyJob?.running} onClick={() => applyNow(applyScope)}>Apply now</button>
              {tab !== 'web' && <button
                className="sm"
                disabled={!!applyJob?.running}
                title="Re-push every gateway, including unchanged ones. Only needed if the cluster drifted from what ProxyCTL applied — a gateway deleted by hand, an apply that failed partway, or one tunnel dead while the rest work. Always safe, just slower."
                onClick={() => applyNow(applyScope, true)}>Full reconcile</button>}
...
function EntryRow({ r, stats, onToggle, onRebind, onDelete, onEdit }) {
  const e = r.entry
        ...
        {stats ? (
          <span style={{ display: 'inline-flex', gap: 16, justifyContent: 'flex-end' }}>
            <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 1 }}>
              ...
              <span style={{ color: 'var(--muted)', fontSize: 10, whiteSpace: 'nowrap' }}>{fmtBytes(stats.rxTotal ?? stats.rxBytes)}</span>
            </span>
            <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 1 }}>
              <span style={{ color: 'var(--ok-tx)', fontSize: 13, whiteSpace: 'nowrap' }}>↑ {rateStr(stats.rTx)}</span>
              <span style={{ color: 'var(--muted)', fontSize: 10, whiteSpace: 'nowrap' }}>{fmtBytes(stats.txTotal ?? stats.txBytes)}</span>
            </span>
          </span>
        ) : '–'}
      </td>
      <td className="row" style={{ flexWrap: 'nowrap', whiteSpace: 'nowrap' }}>
        <button className="sm" onClick={() => onEdit(e)}>Edit</button>
...
function KeysCard() {
  ...
  if (!cfg) return null
  const dirty = cleanFolder(val) !== (cfg.basePath || '')
  const onOther = cfg.gatewayKeysOnOtherPath || 0

  async function save() {
    setBusy(true)
    const { ok, data } = await putJSON('/api/keys-config', { basePath: cleanFolder(val) })
    setBusy(false)
    ...
    <div className="card" id="keysCard">
      ...
      <div className="row" style={{ alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        ...
        <input
          ...
          onBlur={e => setVal(cleanFolder(e.target.value))}
