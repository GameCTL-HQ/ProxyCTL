...
export default function Admin({ onSignOut, onOpenSetup }) {
  ...
  // lockdown, fail2ban never sees anything (see Fail2banPanel), so that
  // tab reads as "WireGuard Access" (adding devices) instead of "SSH
  // Security" (which reads as fail2ban, the thing that's now hidden there).
  ...
  return (
    <>
      ...
      <main>
        <h2 className="section">{tab === 'security' ? (droplet?.sshTunnelOnly ? 'WireGuard Access' : 'SSH Security') : 'Tunnels'}{' '}
        ...
        {/* Tab split: raw L4 game ports (WireGuard droplet) vs L7 host-routed
            ...
            label depends on the lockdown mode: under tunnel-only, fail2ban
            never sees anything (see Fail2banPanel) so the tab is really just
            "add a device to SSH in" — under Option B / no lockdown, fail2ban
            is the active piece, so it keeps the security framing.
            Subtitled so the transport + HTTP distinction is visible
            without opening either tab. */}
        <div className="row" style={{ gap: 8, margin: '0 0 18px' }}>
          {[
            ...
            droplet?.sshTunnelOnly
              ? ['security', 'WireGuard Access', 'add devices · SSH']
        ...
        <div className="card">
          ...
          <form className="wiz" onSubmit={submitEntry}>
            ...
            <div className="step">
              ...
              <p className="hint" style={{ margin: '8px 0 0' }}><span className="mono">port:proto</span> comma-separated, proto = <span className="mono">tcp</span>/<span className="mono">udp</span>/<span className="mono">both</span>. Auto-filled from the template — tweak if your service is customised.</p>
            </div>
        ...
        {/* Domains / gateway keys / Apply — all about the game+web tunnels,
            not relevant while looking at the security tab. */}
        {tab !== 'security' && (<>
        <div className="card">
