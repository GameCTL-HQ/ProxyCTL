...
import { apiFetch, apiJSON, del, postJSON } from './api.js'
...
export default function Admin({ onSignOut, onOpenSetup }) {
  ...
  async function applyNow() {
    if (!await ask("Apply now? Brings up each game's own gateway (~30–90s for new ones), reconciles the droplet peers live (others stay up), and — if Cloudflare is configured — creates/updates each grey-cloud A record. Runs ssh + kubectl with your ambient credentials.", { ok: 'Apply' })) return
    ...
    <>
      <header className="app">
        <div className="bar">
          ...
          <span className="badge" style={{ marginLeft: 10 }}>{data.mode ? 'apply-mode: ' + data.mode : ''}</span>
        </div>
      </header>
