...
export default function PersonalAccessPanel({ onOpenSetup }) {
  ...
  const [wgUpOS, setWgUpOS] = useState('server') // which wg-quick activation snippet to show — 'server' (inline paste) is the universal default (works headless + desktop)
      ...
      {config && (
        <div style={{ marginTop: 18 }}>
          ...
          {(() => {
            ...
            // Headless server (Ubuntu/Debian/Arch/etc., no browser): no file
            // to download — the config is written inline via a quoted
            // heredoc, so the WHOLE thing is one copy-paste into the SSH
            // console. 'PROXYCTL_EOF' is quoted so nothing in the config is
            // shell-expanded. Works on any distro with wireguard-tools.
            const serverCmd = `sudo wg-quick down proxyctl 2>/dev/null; sudo rm -f /etc/wireguard/proxyctl.conf\n` +
              `sudo mkdir -p /etc/wireguard\n` +
              `sudo tee /etc/wireguard/proxyctl.conf >/dev/null <<'PROXYCTL_EOF'\n` +
              `${config.text}\n` +
              `PROXYCTL_EOF\n` +
              `sudo chmod 600 /etc/wireguard/proxyctl.conf\n` +
              `sudo wg-quick up proxyctl\n` +
              `sudo wg show proxyctl` + sshCmd
            ...
            const cmdFor = { server: serverCmd, linux: linuxCmd, mac: macCmd, windows: winCmd }
            const hintFor = {
              server: <>One copy-paste — no download needed. The <span className="mono">tee … &lt;&lt;'PROXYCTL_EOF'</span> heredoc writes the config straight to <span className="mono">/etc/wireguard/proxyctl.conf</span> from the pasted block, so this works over plain SSH on a headless box (Ubuntu Server, Debian, Arch, …). Just needs <span className="mono">wireguard-tools</span> installed (<span className="mono">apt install wireguard-tools</span> / <span className="mono">pacman -S wireguard-tools</span>).</>,
              linux: <>Assumes the <span className="mono">.conf</span> above was downloaded to <span className="mono">~/Downloads</span> (adjust the path otherwise) and <span className="mono">wireguard-tools</span> is installed. No browser on the box? Use the <strong>Linux server (paste)</strong> tab instead — it needs no download.</>,
              mac: <>Assumes Homebrew <span className="mono">wireguard-tools</span> and the <span className="mono">.conf</span> downloaded to <span className="mono">~/Downloads</span>. The official WireGuard app imports the tunnel via its GUI instead.</>,
              windows: <>Windows imports the tunnel through the WireGuard app's GUI, then this connects over SSH.</>,
            }
            return (
              <div className="wstep" style={{ marginTop: 16 }}>
                <p className="st">2 · Activate the tunnel on {config.name}, then connect</p>
                <div style={{ display: 'flex', gap: 6, marginBottom: 6, flexWrap: 'wrap' }}>
                  <button className={'sm' + (wgUpOS === 'server' ? ' primary' : '')} onClick={() => setWgUpOS('server')}>Linux server (paste)</button>
                  <button className={'sm' + (wgUpOS === 'linux' ? ' primary' : '')} onClick={() => setWgUpOS('linux')}>Linux desktop (.conf)</button>
                ...
                <CmdBlock text={cmdFor[wgUpOS] || serverCmd} toast={toast} />
                <p className="hint" style={{ margin: '6px 0 0' }}>
                  Same block whether this is a fresh setup or replacing an earlier one — tearing down an interface/file
                  that doesn't exist yet is a harmless no-op. {hintFor[wgUpOS]}
                </p>
              </div>
            )
          })()}
