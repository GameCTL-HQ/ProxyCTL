...
export default function SetupWizard({ onFinish, onSignOut }) {
  ...
  // "Just use the base NFS": clusters already running an NFS-subdir
  // provisioner (a StorageClass like nfs-ssd) expose the backing server +
  // export — prefill the share with it (plus a /ProxyCTL dir) so a fresh
  // install is Test → Move in with zero typing. Prefill only while both
  // fields are untouched; the operator's own input always wins.
  useEffect(() => {
    if (storage?.mode !== 'ephemeral' || !keysCfg) return
    if (keysSrv.trim() || keysExp.trim()) return
    if (keysCfg.discoveredNfsServer && keysCfg.discoveredNfsExport) {
      setKeysSrv(keysCfg.discoveredNfsServer)
      setKeysExp(keysCfg.discoveredNfsExport.replace(/\/$/, '') + '/ProxyCTL')
    }
            ...
            <div className="step on">
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                <input className="mono" style={{ flex: '2 1 220px' }} spellCheck="false"
                  ...
                  placeholder={keysCfg?.discoveredNfsExport || 'e.g. /mnt/storage/ProxyCTL'} />
              </div>
              {keysCfg?.discoveredNfsExport && (
                <p className="hint" style={{ marginTop: 6 }}>
                  Prefilled from your cluster's existing NFS provisioner (StorageClass{' '}
                  <span className="mono">{keysCfg.baseStorageClass}</span>) — the same disk your PVCs
                  already land on, in a dedicated <span className="mono">ProxyCTL</span> directory.
                  Change it if you want a different location.
                </p>
              )}
