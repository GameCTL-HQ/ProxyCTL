...
import { cleanExport, cleanFolder, joinPath } from './paths.js'
...
export default function SetupWizard({ onFinish, onSignOut, initialStep }) {
  ...
  async function probeShare() {
    const srv = keysSrv.trim(), exp = cleanExport(keysExp)
  ...
  async function saveKeys() {
    if (!cleanFolder(keysVal)) { toast('Enter a folder for the keys', true); return false }
    const srv = keysSrv.trim(), exp = cleanExport(keysExp)
    ...
    const { ok, data } = await putJSON('/api/keys-config', {
      basePath: cleanFolder(keysVal), nfsServer: srv, nfsExport: exp,
    })
    setKeysBusy(false)
            ...
            <div className="step on">
              ...
              <div className="notice" style={{ marginBottom: 18 }}>
                ...
                Watches sshd's own auth failures and firewalls off anyone hammering it, independent of whichever SSH restriction you land on below. {fail2ban.installed
                  ? <>Bans after <strong>{fail2ban.maxRetry} failed attempts</strong> within <strong>{fail2ban.findTime}</strong> — {(!fail2ban.banTime || fail2ban.banTime === '-1')
                      ? <><strong>permanently</strong>, first offense, no temporary grace period</>
                      : <>banned for <strong>{fail2ban.banTime}</strong></>}. Tune or switch this off any time from the main page's fail2ban card. {fail2ban.active ? 'Active.' : '⚠ installed but not active.'}</>
                  : <>Not installed yet.</>}
                <div style={{ marginTop: 10, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div className="step on">
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                <input className="mono" style={{ flex: '2 1 220px' }} spellCheck="false"
                  ...
                  onBlur={e => setKeysExp(cleanExport(e.target.value))}
              ...
              <div style={{ marginTop: 14, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                <button className="primary"
                  disabled={adoptMsg?.pending || !(probeMsg?.ok && probeMsg.srv === keysSrv.trim() && probeMsg.exp === cleanExport(keysExp))}
          ...
          {cur === 'storage' && storage?.mode !== 'ephemeral' && (
            <div className="step on">
              ...
              <input className="mono" value={keysVal} onChange={e => setKeysVal(e.target.value)}
                onBlur={e => setKeysVal(cleanFolder(e.target.value))}
              ...
              {keysCfg && (
                <p className="hint" style={{ marginTop: 8 }}>
                  ...
                  {(() => {
                    ...
                    const exp = cleanExport(keysExp || keysCfg.discoveredNfsExport || '')
                    const folder = cleanFolder(keysVal || keysCfg.defaultBasePath)
                    return exp
                      ? <>Saves to <span className="mono">{joinPath(exp, folder)}</span>{srv && <> on <span className="mono">{srv}</span></>}</>
              ...
              <div style={{ marginTop: 14, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                <button className="primary" disabled={keysBusy || !cleanFolder(keysVal)} onClick={saveKeys}>Save location</button>
