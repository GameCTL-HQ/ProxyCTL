...
export default function SetupWizard({ onFinish, onSignOut }) {
  ...
  async function next() {
    // On the keys step, persist a changed folder/share before advancing so the
    // choice made during setup actually sticks.
    if (STEPS[step] === 'keys' && keysVal.trim() && keysDirty()) {
      await saveKeys()
    }
        ...
        <div className="body">
          ...
          {cur === 'keys' && (
            <div className="step on">
              ...
              <p className="lede">Each tunnel ProxyCTL creates self-generates a <strong>WireGuard keypair</strong> (a private + public key) that's persisted on NFS so the tunnel survives restarts — and so you can find the keys on the share later if you ever need them. Choose the share and folder they live under.</p>
