...
> Multi-account Cloudflare tokens now work. "Set up Cloudflare Tunnel" failed with an Authentication error (code 10000) when the API token spans more than one Cloudflare account — ProxyCTL derived the tunnel account from the first zone it could see, which is often a *different* account than the one holding Tunnel permission. It now probes each visible account until it finds one the token can read.

### Fixed

- **Tunnel account detection for multi-account tokens** — With a zone-scoped token, ProxyCTL derives the Cloudflare account from the zones the token can see, and assumed those zones all live in one account, taking the first zone returned (alphabetical). For tokens that span multiple accounts (personal + work, zones under different organizations), that picked the account *without* Tunnel permission, so "Set up Cloudflare Tunnel" died with `{"code":10000,"message":"Authentication error"}` — misleading, because the token does have Tunnel permission, on a different account. It now collects the distinct account IDs across all visible zones, probes each with a read-only tunnel-list call, and uses the first one the token can actually read. Single-account tokens behave exactly as before.

## [v0.6.19] - 2026-08-21

> A loading state on first load. Opening or refreshing the site no longer flashes "No entries yet" in the Proxy Entries and Web Apps tables while the first fetch is still in flight — a spinner shows instead until the data arrives.

### Fixed

- **Loading state instead of a false empty state** — On first load, the Proxy Entries and Web Apps tables rendered "No entries yet — add one below" while `/api/entries` and `/api/webroutes` were still in flight, which read like the entries had vanished. Both tables now show a spinner with "loading entries…" / "loading web routes…" until the first fetch completes; the genuine empty state (or the rows) appears after. A later failed refresh doesn't flip the tables back to the loading row.

## [v0.6.18] - 2026-08-21

> Persistent traffic accounting and a WAN egress quota. Byte counters now survive restarts and gateway re-keys, the droplet's outbound traffic — the part DigitalOcean bills — is tracked against a configurable allowance with Discord alerts at "nearing" and "over", and the quota bar moves live at the UI's poll rate. The sampler is split so counter reads run every 10s while probes and history keep their 60s cycle.

### Added

- **Persistent traffic accounting** — Per-entry WireGuard byte counters and the droplet's WAN counters reset on every restart and gateway re-key, so the totals only ever described the current uptime. They now accumulate across both: each sampler tick reads the live counters, adds the delta since the last reading (with reset detection for when a counter wraps to zero), and atomically persists the running totals to /data. The stats endpoint and the UI show lifetime totals, not since-boot totals.
- **WAN egress quota with Discord alerts** — A configurable outbound allowance (default 500 GiB, the standard DigitalOcean data plan) tracks billable WAN egress only — WAN receive is not billed and never touches the quota. Crossing the nearing threshold (default 85%, configurable) sends a yellow Discord embed; crossing the quota itself sends a red one; dropping back under after a reset or a quota increase sends the "back under" notice. The billing cycle auto-resets on a day-of-month you choose, and a manual reset zeroes the WAN and per-entry totals.
- **Quota configuration in Monitoring settings** — Quota size (the field now shows its unit), the nearing percentage, and the reset day live in the alerts card; the Proxy Entries view shows the quota bar with a manual reset.
- **Live quota numbers** — /api/stats reads the WAN counters in the same SSH call as the sampler and returns the persisted total plus the in-flight delta since the last tick, so the quota bar moves at the UI's 5s poll rate instead of stepping once per sampler tick.

### Changed

- **Merged traffic card** — The separate WAN egress and Totals cards are one labeled card: "WAN egress — what DigitalOcean bills" (the quota bar) over "Through proxy entries — WireGuard only" (rx/tx/connections), with a single refresh hint.
- **Split sampler cadence** — The SSH byte-counter read, the traffic record, and quota evaluation now run every 10s; the heavier reachability/latency checks, web-route probes, and uptime-history appends stay on the 60s cycle (every 6th tick). History retention and probe load are unchanged.

### Fixed

- **Per-row UP total matches the card totals** — The per-row UP column read a since-boot counter while DOWN and the top totals read the persistent one, so after a restart the rows no longer added up to the card. The row now reads the same persistent total.
- **Quota alerts fire for every crossed threshold** — A single tick that jumped usage from ok straight to over (small test quota, traffic spike) skipped the nearing alert. Level transitions now emit every level crossed, in order — near, then over. Alert sizes are human-readable (MB/GB) and the quota card carries a refresh hint.
- **Row action buttons stay on one line** — The per-row buttons wrapped to a second line on narrow screens; they now stay on one row.

### Security

- **postcss build dependency overridden** — Vite's transitive postcss resolved to 8.5.15, affected by GHSA-r28c-9q8g-f849 (high: source-map path traversal) and GHSA-fxqj-rqcc-2cmp (medium: incomplete fix of the same class). An npm override pins postcss to ^8.5.23 (the lockfile resolves 8.5.26). PostCSS is build-time only and is never bundled into the app or the image.
